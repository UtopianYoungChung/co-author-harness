...
**Coverage regression hook (cross-round, S4.5-deferred consumer).** `last_coverage_score` is persisted across rounds at S4 to **forward-compatibly support** cross-round regression detection on Ph3 retraction → Ph2 re-entry paths. The consumer logic — comparing the post-audit score against the prior round's `last_coverage_score` and emitting a non-blocking advisory line in the cycle log when regression exceeds the `coverage_regression_floor` parameter (default 0.05) — **lands at S4.5** alongside the parameter's surfacing in `reviews/classification.md` template (and concurrent with the synthesis-alignment threshold and other coverage-tuning parameters per the §6.6 stage scope). At S4 the field is **written but not yet consulted**; the architecture §5.4 stated consumer is staged for S4.5, not delivered at S4. This is the documented-future-feature framing — readers should not assume the regression-detection logic is live at S4 tip.
