...
Before any academic Generator dispatch, read `references/ASSIGNMENT_MILESTONE_PROCESS.md`, read the controlling brief in full, and resolve and verify `reviews/assignment_contract.json`. Derive the active assignment target as the first applicable M1-M4 record whose **top-level `record.status`** is not `accepted`, reading `reviews/phase_state.json`. Two different fields carry two different facts, and conflating them inverts the predicate: the milestone's own status is `record.status: accepted`, while its **approval evidence** separately records `record.approval.status: approved`. An accepted record therefore carries `status: accepted` AND `approval.status: approved`. Deriving the target from `approval.status != "accepted"` would match every correctly approved milestone — because a correct approval never says `accepted` — and so would always select M1. For draft targets, run `python scripts/assignment_process_gate.py --project-root <project-root> --stage draft --target-milestone <target> --emit-receipt reviews/.harness/assignment/ready/gate_receipt_<target>_<utc>.json`; if M1-M4 are accepted, final-paper or Ph4 dispatch runs the same gate with `--stage final` and a FINAL receipt. The assignment defines the deliverables; M1-M5 are machine slots and must not overwrite the instructor's names or functions.
