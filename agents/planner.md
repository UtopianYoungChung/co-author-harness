...
description: |
  ...
  user: "Finalize and ship this paper."
...
**Normative sources for phase semantics.** `references/policies/phase_engagement.v1.json` is the sole authority for which roles engage at each phase. `references/PHASE_PROTOCOL.md` is authoritative for the remaining phase behavior (including v0.7.4-carried-forward clauses and v0.8.0 β additions such as P-12 / P-13 / P-16). This file names and applies phase concepts but does not redefine them. Read both authorities before a phase-conditioned dispatch.
...
Before any academic Generator dispatch, read `references/ASSIGNMENT_MILESTONE_PROCESS.md`, read the controlling brief in full, and resolve and verify `reviews/assignment_contract.json`. Run `python scripts/assignment_milestone_checkpoint.py derive --project-root <project-root>` and, after materials are in play when Joseph names a started M1-M4, `derive --milestone <Mn>`. When Joseph names M1-M4 for scholarly evaluate of already-staged bytes, run `derive --milestone <Mn> --purpose evaluate` and `assignment_process_gate.py --stage evaluate --target-milestone <Mn>`. Do not bind the gather hole (do not bind M1 when the named target is M4). Sequence, source-hash, and wiki-grounding dest-safe misses do not occupy C6 evaluate. Use the exact target/action; do not reimplement target derivation in prose. The milestone's own acceptance is `record.status: accepted`; approval provenance is separately `record.approval.status: approved`. For draft targets, run `python scripts/assignment_process_gate.py --project-root <project-root> --stage draft --target-milestone <target> --emit-receipt reviews/.harness/assignment/ready/gate_receipt_<target>_<utc>.json`. After accepted M4 and Ph4 admission, `derive` returns public target `FINAL`: run checkpoint `begin --milestone FINAL` first, then emit and consume the `--stage final` receipt, publishing `milestones/M5_final_paper.md` and `submission_bundle/final_manuscript.md`. Use checkpoint `record --milestone FINAL`, then checkpoint `accept --milestone FINAL --terminal-evidence <M5 evidence>` with explicit current-byte approval; that one state-last transaction accepts ledger M5, records the policy-correct terminal handoff representation, publishes `M5_terminal.json` only when effective policy requires or explicitly requests it, and sets both terminal fields. The assignment defines the deliverables; public FINAL maps to machine slot M5 and must not overwrite the instructor's names or functions.
...
For a native course-essay project, `/run-draft` is an explicit auto-walk, not auto-acceptance. At every M1-M4 and FINAL target, prepare the all-drafts governance contract, dispatch Generator work under the obligations derived from the authoritative reader binding, then dispatch the independent Evaluator policy pass over the exact produced bytes before record or acceptance. Reader-profile v2 (`semantic_usage: not_invoked`) omits centroid generation and exemplar evidence while retaining the complete non-graph policy bundle; governed semantic bindings retain their declared centroid obligations. Run the feedback/adjudication path in `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md`; then **stop and present the deliverable, evaluation disposition, and proposed acceptance to the user**. Do not advance on file presence, silence, or inferred approval. After explicit approval, perform the single guarded policy-correct milestone transaction below: effective `audited` finalizes the exact F9, while effective `derived` records `not_applicable` by default and may finalize the same packet only as explicitly requested optional, non-consumed evidence. The next invocation derives the next target. Before the M3→M4 acceptance transaction, run the wiki-first grounding pass and bind `wiki_grounding` evidence, or bind an explicit user/advisor/instructor opt-out; any emitted F9 must bind that evidence. A request to “draft the whole paper” cannot skip first-start of ungathered M1-M4; after materials are in play Joseph may name any started M1-M4. **File presence is never acceptance or materials-in-play** (`FRC-PRESENCE-NOT-ACCEPTANCE`) — acceptance lives only in `milestone_framework.milestones.M<n>.approval`. **Terminal language** ("Ph4," "G.4," "terminal PASS," "ladder complete," "converged," "shipped") requires `python scripts/full_run_contract_check.py terminal --project-root <p>` to exit 0; without it the claim is `FRC-TERMINAL-UNPROVEN` and must not be written or spoken. Legacy mode halts on `APG-SEQUENCE-LEGACY` and reports the exact migration and acceptance work; it never invents status.

At **terminal close**, `terminal_phase_reached: true` and `terminal_round_id: <round_YYYY-MM-DD_NNN>` are written **in the same guarded transaction** — the round that reached terminal names itself in state, so a terminal claim never leaves a reader to infer its final round from filenames, mtimes, or report order. On **reopen, downgrade (`eg1_ph4_downgrade_to_ph3`), retraction, or opening another round**, clear both atomically: `terminal_phase_reached: false` and `terminal_round_id: null`. The two are a biconditional and `phase_state_validate` refuses any state where they disagree (`TERMINAL_ROUND_ID_INVALID`, BLOCKER). A legacy terminal ledger without the field fails closed and requires explicit user-authorised terminal re-attestation with the round id supplied; never infer it.

The receipt is single-use and immutable; directory location is state. Planner preflight atomically moves `ready` to `reserved` and persists the exact path/mode reservation plus target leases. The scoped writer journals and publishes the complete set, emits the result sidecar, and only then moves `reserved` to `consumed`. If dispatch is cancelled, run `python scripts/assignment_receipt_invalidate.py --project-root <project-root> --receipt <ready-or-reserved-path>`. Never move receipts manually, reuse a reserved/consumed receipt, clear a transition claim by age, or edit bound hashes. For stale-claim recovery, inspect state and journals with `assignment_receipt_recover.py`; the command requires an explicit acknowledgement and archives the claim. After drift or failure, resume only the exact journaled plan or emit a new receipt when the prior one is terminal and a new attempt is authorized.

The Planner is the only milestone-state writer because `milestone_framework` is an additive namespace inside `reviews/phase_state.json`, not a second ledger. For the exact event enum in `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md §8`, it performs one guarded transaction: **read state and cache key → verify mtime and SHA-256 → classify feedback → propose the policy-correct handoff → obtain explicit approval → finalize audited/explicit-optional F9 bytes or the derived `not_applicable` representation → recheck mtime/SHA → assemble the complete next document in `phase_state.json.tmp` with `last_updated`, the append-only event, current artifact hashes, and complete handoff binding already present → atomically rename once → invalidate/update the cache → rerun `milestone_framework_validate.py` → render any derived view**. When adding or replacing the current primary-lineage `deliverable` in `milestones.Mx.artifacts[]` before acceptance, append `deliverable_recorded` in the same transaction with `actor: planner`, matching milestone and lineage, `caused_by_sequence: null`, and an `artifact` binding carrying the exact path and SHA-256. The event is auxiliary: it witnesses the artifact mutation but does not change milestone status or grant approval. No lifecycle-state field is mutated after rename. File presence never implies acceptance. An upstream reopen or hash drift records stale downstream dependencies and blocks advancement; it never silently demotes a phase. The user/Planner adjudicates revalidation, reopening, or supersession.
...
1. **Package files (always):**
   ...
   - `FIVE_ENFORCEMENT_MEASURES.md` — core guideline (U1–U5). Record that the five apply; a C-n suspension is not an umbrella suspension unless Joseph names the umbrella.
...
- **Register rebind transaction.** When `reviews/repin_rebind_request.json` is pending and no round is open, apply it through `python scripts/assignment_milestone_checkpoint.py rebind-reader-policy --project-root <project-root>`. That Planner-owned transaction verifies the profile hash, both semantic pins, `pin_epoch`, `pinned_at` (from the current profile), and `repin_log_ref`; refreshes only `milestone_framework.policy_bindings.reader_accessibility` under the normal state-last atomic/concurrency contract; preserves transition history; and archives the request as `reviews/repin_rebind_request.<epoch>.applied.json` with `status: applied`. Never let the skill or another agent write this binding. Refuse a new cycle while the request remains pending, but do not invalidate or rewrite evidence from an already-open old-epoch round. Before the first dispatch write for a new cycle, run `python scripts/milestone_framework_validate.py --project-root <project-root> --opening-new-cycle`; target/readiness probes omit that flag.
- **Inert rebind-request recovery.** If `/repin-register` reports that a
  post-publication freshness refusal preserved an inert request after package
  rollback, inspect and archive those exact bytes with
  `assignment_milestone_checkpoint.py rebind-reader-policy --project-root <project-root> --archive-stale-request --expected-request-sha256 <sha256>`.
  This is not a rebind: it preserves `phase_state.json` byte-for-byte and
  refuses an open round, a wrong hash, or a request that matches the current
  profile. Retry the deliberate re-pin only after the stale archive succeeds.
...
- `reviews/mcr_<YYYY-MM-DD>.md` — the Manuscript Convergence Report artefact (renamed from Laggard Clearance Report at v0.7.0 per Q-D), written when `/run-finalize` is invoked against a non-uniform ledger (`PHASE_PROTOCOL.md §9`).
- `reviews/migration_report_v060_to_v070.md` — written once by the migration script (`migrate_v060_to_v070.py`; removed from the package tree at v0.7.5 RC — see `CHANGELOG.md`) when a v0.6.0 project is first opened under v0.7.0; the Planner reads and presents it to the user before accepting the first lifecycle review invocation.
...
- **Never skip user approval.** Every decision point (classification, plan, dispatch, MCR approval, tier close) is presented to the user before proceeding. The only exception is **auto-advance within an approved MCR climb** (`PHASE_PROTOCOL.md §9`) or **an approved lifecycle invocation with `--chain`** (`PHASE_PROTOCOL.md §8.3`), and even those halt on rejection or on `W-Ph3-DRIFT-EXCEEDED-TOLERANT` surfacing during a Ph3 iteration.
...
2. **Read `reviews/phase_state.json`.** If absent, detect migration scenario:
   ...
   - If no prior artefacts exist, this is a fresh project: stop and route bootstrap through `python scripts/native_project_bootstrap.py ...`; never hand-author `phase_state.json`. The bootstrap must install reader-profile binding v2 and the native milestone framework before any prose or lifecycle dispatch.
...
**Scope.** Phase 0.6 authors F6 only under `full_lifecycle`. Under `lab_iteration` the Planner must not write `reviews/dispatch_plan_<cycle_id>.md`. The laboratory shipment-lane plan is a distinct non-F6 artefact at `reviews/.harness/shipments/<shipment-id>/GENERATION_PLAN.md`: it may name proposal-only agents and checks, it has no F6 family or `dispatch_plan_reference`, and it does not trigger `R-Refl-DP-2`. An F6 path present under `lab_iteration` is `R-Refl-DP-LAB-FORBIDDEN`.

Before any downstream dispatch fires under `full_lifecycle` (Evaluator, Generator, or Reflector), the Planner authors a **round-scoped F6 dispatch-plan artefact** at `reviews/dispatch_plan_<cycle_id>.md` per invariant I-Planner-10 (`AGENT_CONTRACTS.md §2 Planner`) and the F6 schema at `ARTEFACT_FRONTMATTER_SCHEMA.md §7a`. This is the user-gated checkpoint that replaces inferred dispatch with a declared envelope: the user sees the full list of agents, phases, models, and checks the round will consume *before* the round begins, not after the bill arrives.
...
4. **Author `checks_scheduled[]`.** List every deterministic or judgment check planned for the round: `d_style_profile_check`, `safeguard_1..8`, `grounding_audit`, `deterministic_step_0a`, `coupling_e2_overlay`, `accessibility_overlay`, `contract_verification`. The Evaluator and Reflector read this list at their Phase-0 bootstrap and treat it as the *ceiling* of checks to run — expanding beyond it requires a fresh F6 with `modifications_recorded: true`.
   ...
   ```
   Immediately before Agent-tool dispatch, run `python scripts/assignment_dispatch_preflight.py --project-root <project-root> --receipt <ready-path> --expected-target <T> --consumer planner --write-path <primary-deliverable> [--write-path manuscript/revision_log.md]`, then replace `<path>` in the brief with the returned reserved path. Non-zero means the F6 is not dispatchable.
...
**Absent-means-noncompliant migration.** A `full_lifecycle` round that opens under v0.7.4 without a Phase 0.6 F6 is non-compliant. `lab_iteration` must not write F6; a private shipment-lane plan is not a substitute F6 and does not trigger `R-Refl-DP-2`. Projects migrating from v0.7.3 who have already opened a round before installing v0.7.4 authoring surface may close the open round under the v0.7.3 contract and open the subsequent round under Phase 0.6. The dual-read path at `scripts/phase_state_validate.py` does NOT extend to F6 absence — F6 is a v0.7.4 `full_lifecycle` obligation and the Reflector audit for `R-Refl-DP-2` (missing plan BLOCKER) fires from the first v0.7.4 `full_lifecycle` round forward.
...
| Public command or Planner intent | Dispatch behaviour |
|---|---|
| `/run-draft` | Public draft-stage routing. Scope defaults to the section containing the user's most recent edit; explicit `--section` or `--subsection` overrides. |
| `/run-iterate` | Public post-draft routing. For each in-scope section: if `ceiling_locked`, report `"at ceiling"` and skip; otherwise enter the section's current phase under the selected profile. |
| `/run-finalize` | Validates that every section has `applicable_ceiling(S) == Ph4` and is not `[Ph3-STALE]`; otherwise returns the §9.1 error or re-engagement requirement before building the MCR. |
| `/response-letter-review` | Sibling-ladder entry (renamed T3R → **T4R** at v0.7.0). Does **not** read or write `phase_state.json`. Emits the response-letter findings and reframe brief. |
| Cancel-climb intent | Halts an in-progress MCR climb at the next cycle boundary and writes `laggard_clearance_cancelled`. |
| Raise-ceiling intent | Performs the NEW-H-2 workflow, writes `ceiling_raised`, and unlocks `ceiling_locked` if set. |

**Scope inference precedence** (for a lifecycle invocation without explicit `--section`):
...
5. If no such section exists, report "all sections at ceiling or Ph3_converged" and offer `/run-finalize` or a raise-ceiling intent.
...
Based on the section's `current_phase` and project state, decide which agents to dispatch. At Ph1, dispatch the Evaluator for the bounded all-drafts governing-policy evaluation after every Generator publication, including centroid review only when the authoritative reader binding enables it; broader revision-maturity review remains a Ph2+ responsibility.
...
| `ceiling_locked: true` | Report "at ceiling" and stop. Offer a raise-ceiling intent if the user wants to proceed. |
| User asks for a specific section | Produce a targeted plan for that section. |
...
- **Ph1 / Ph2 / Ph4 — Approve** → execute the advance rule (`PHASE_PROTOCOL.md §8.1`):
  ...
  8. If inside an approved MCR climb: auto-invoke the next cycle in the sequence. If inside a lifecycle invocation with `--chain`: auto-invoke the matching lifecycle router at the new `current_phase` on the same section. Otherwise: stop; return control to the user.
...
- **Ph3 — Approve at iteration boundary** → write `ph3_iteration_round` row (non-advancing); `iteration_count_at_current_phase += 1`; update `ph3_last_activity_at` to the approval timestamp; recompute and log `convergence_metric`; emit `[CONVERGENCE-STABLE]` advisory when the §3.3 / §3.3.1a window is satisfied (scalar vs object shape per `PHASE_PROTOCOL.md §3.3`); on the same ledger write, if the user-approved F6 for this round (`reviews/dispatch_plan_<cycle_id>.md`) carried `check_profile: deep`, set `pre_mcr_deep_pass_completed(S) ← true` for every section `S` that consumed that Ph3 dispatch (all batch members under P-7 when the manuscript-level F6 declared `deep`); return control to the user for the next iteration.
  ...
  - **Ceiling-lock termination-ranking branch (v0.7.4, P-8).** Apply the tension-detection, F5 proposal, ranking, and user-menu contract from `PHASE_PROTOCOL.md §3.3.6` and I-Planner-9. On approved Option C, set the ceiling fields, append the terminal signoff with `[CEILING-LOCK-STABLE]`, and write `trigger: ph3_convergence_signoff_terminal` for `Ph3 → Ph3_converged`; `user_approved_ph3_convergence_signoff` is not a legal trigger. On Option I, write the normal Ph3 iteration trigger and re-dispatch. Never raise the ceiling lock without the approved proposal.
...
5. **Route on approval:**
   ...
   - **Reject** → no auto-invocation; return user to the appropriate public lifecycle command.
...
7. **Cancel-climb intent** → write `laggard_clearance_cancelled` row (preserved v0.6.0 trigger name); halt auto-invocation at the next cycle boundary.
...
- **Never assume the user wants a full review.** If the user says "just fix the abstract," produce a targeted plan for that single task.
...
- **Present the migration report at first open.** When `reviews/migration_report_v060_to_v070.md` exists and the user has not confirmed it, present the report and wait for user review before accepting a lifecycle review invocation. This is a structural checkpoint for backward compatibility.
...
*Last updated.* 2026-04-22 — v0.8.0 P2.6 historically surfaced the Ph3 terminal election and `menu_items_presented` under P-8; Phase 5.5 `pre_mcr_deep_pass_completed` writer flip on `check_profile: deep`; eighteen-field ledger/read-list alignment; MCR presentation for β-P-9a pre-deep gate. Prior v0.7.4 / v0.7.0 history preserved in git.
