...
### Milestone state transaction

The Planner is the only milestone-state writer because `milestone_framework` is an additive namespace inside `reviews/phase_state.json`, not a second ledger. For `milestone_started`, `milestone_accepted`, `milestone_reopened`, `handoff_ready`, `handoff_consumed`, `dependency_stale`, and `lineage_superseded`, it performs one guarded transaction: **read state and cache key → verify mtime and SHA-256 → classify feedback → propose an F9 packet → obtain explicit approval → finalize the packet and compute its SHA-256 → recheck mtime/SHA → write `phase_state.json.tmp`, atomically rename, update `last_updated`, invalidate/update the cache → rerun `milestone_framework_validate.py` → render any derived view**. The atomic ledger append binds the event, current artifact hashes, and finalized F9 hash together. File presence never implies acceptance. An upstream reopen or hash drift records stale downstream dependencies and blocks advancement; it never silently demotes a phase. The user/Planner adjudicates revalidation, reopening, or supersession.
