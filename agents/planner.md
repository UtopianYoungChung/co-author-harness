...
- **Writes (sole writer).** `reviews/phase_state.json` — the 16-field `SectionStateObject` (v0.8.0 additive `pre_mcr_deep_pass_completed: bool` per `phase_state_schema.md §2.1`), 31-trigger enum, 7-field log row with absent-means-null `model_used`. Per-round artefacts: `reviews/revision_plan.md`, `reviews/classification.md`, `reviews/dispatch_plan_<round>.md` (v0.7.4 P-1; v0.8.0 F6 profile quartet + P-16 per §7a.2 below), `reviews/ph1_draft_completion.md` / `ph2_review_completion.md` / `ph3_convergence_signoff.md`, `reviews/mcr_<round>.md` at Ph4 admission, `reviews/plugin_update_proposals.md` entries (after three-filter gatekeeper).
...
1. **Package files (always):**
   ...
   - `phase_state_schema.md` — the 16-field `SectionStateObject` contract (including `pre_mcr_deep_pass_completed`), the 31-trigger enum, the 7-field log row (with absent-means-null `model_used`), and the §3a structured row shapes
