...
**Check 8 sub-check enumeration (v0.10.1).** Check 8 carries eight Sub-checks (A–H), not seven: A–F (local-scale: cadence, rhythm, first-use, signpost, jargon density, worked-example), G (cumulative-scale: consolidation anchors at structural boundaries; T3/T4 only), and H (register-scale: register appropriateness within structurally-required non-technical passages; passage-scoped subset runs at Ph2 because the five non-technical passage roles are section-resolvable). When dispatching `accessibility-overlay` (the Check 8 judgment-layer subagent per §Sub-pass delegation below), you do not enumerate sub-checks in your envelope — the overlay reads `register_class` from `research_notes/directives.md` (default `technical`) and resolves H scope itself. Cite the overlay's `reviews/safeguard_check8_<date>_<cycle_id>.md` artefact by path in your consolidated findings report (I-Eval-7); do not re-adjudicate H findings inline. Sub-check G remains under `advisory_until: next_manuscript_at_ph3` for pre-2026-04-23 manuscripts; Sub-check H is under `advisory_until: H_two_revision_cycles` at v0.10.1 introduction (cycle-counting via `h_advisory_cycles_observed` in `reviews/classification.md`). Both flags are honoured by the overlay's aggregate-verdict computation; the §3.3.3 TerminalSignoffRow gate reads the overlay's aggregate verdict directly and does not need separate flag-state arithmetic.
...
| **Ph2 Review & Revise** | Full local pass on the section envelope. SD/SR read-prerequisite **iff** `sd_sr_required: true` in `reviews/classification.md` (`PHASE_PROTOCOL.md` §3.1.2); absent means false (v0.7.1 default). | Step 0 + 0a + 2 or 3 (section-scoped) + 7 (local) | **1, 4, 5, 8** (Check 8 advisory on Ph2 admission; carries to Ph3 TerminalSignoffRow gate per §3.3.3) | No | Consolidated findings (abbrev.); per-step findings |
...
**Ph2 SD/SR rule (binding when opted in).** When `sd_sr_required: true`, the goal-model artefact named in `phase_deliverable_path` or `directives.md` **must** be read before the prose. Missing / unreadable → **`E-Ph2-SD-UNGROUNDABLE`** (MAJOR default; BLOCKER if argumentation is explicitly goal-model-derived). When the flag is absent or false, the sub-step skips; that finding class cannot fire.
...
- **Scope budget (v0.7.4 ladder).** At **Ph1 Plan & Draft**: dormant — no evaluator run. At **Ph2 Review & Revise**: Step 0 + Step 0a + Step 2 or 3 (scoped to the section envelope per the `heading_path`) + Step 7 (local items) + SAFEGUARD 1/4/5, with SD/SR model read-prerequisites enforced **only when `sd_sr_required: true`** in `reviews/classification.md` (`PHASE_PROTOCOL.md §3.1.2`); the v0.7.1 default `sd_sr_required: false` reduces Ph2 to prose-only reading. At **Ph3 Iterate & Converge**: Steps 0–8.5 per iteration plus a Step 8.3 convergence-log entry; unbounded iteration count; Coupling E.2 overlay optional. At **Ph4 Finalize & Close**: strict superset of Ph3, Coupling E.2 overlay mandatory, G.4 sign-off mandatory, external verifiers required per `directives.md`. Do not re-run Steps 1–7 on the same text twice in the same round; after Generator edits at Ph3/Ph4, run a re-check (SAFEGUARD Check 1 + spot-checks), not a full re-pass (see `AGENT_CONTRACTS.md` §2).
