...
**2. Schema version check.** Verify `schema_version == "0.7.4"`. If the ledger is at v0.7.0–v0.7.3 (i.e., carries the phase-named schema but predates the v0.7.4 row-widening), emit a `DEPRECATION_WARNING` finding: `[LEDGER-NOT-MIGRATED-V074]` and point to `migrate_v073_to_v074_tier_to_phase.py` (removed from the package tree at v0.7.5 RC; see `CHANGELOG.md`). If the ledger is still at v0.6.0 (tier-named schema), emit a BLOCKER finding: `[LEDGER-NOT-MIGRATED]` and stop further audit (the migration must run first). Cite `migrate_v060_to_v070.py` as the remedy, followed by the v0.7.4 migration.
