...
- **Universal centroid scope.** For every M1-M4 and FINAL deliverable or revision, run the `write` centroid derivation before drafting and bind the complete applicable draft-governance policy. Retrieved Yu near-neighbors may condition the domain-native surface register at every milestone. Dennett may condition argument architecture only when admitted with `warrant_scope: argument-only`; never use Dennett as a surface-register or voice-emulation target. A pending/unadmitted Dennett source is omitted with `APG-EXEMPLAR-DENNETT-PENDING` and does not block Yu-conditioned drafting. Artifact presence never waives this obligation.
