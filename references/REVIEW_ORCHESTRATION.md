...
### 3.3 By phase (v0.8.0 Lifecycle-Phase Ladder; vocabulary migrated 2026-07-07 — tier-era codes like `E-PSTAGE-REQUIRED-AT-T2` and the `/t3-reengage` command name are stable identifiers and keep their historical spellings)
...
|---|---|---|---|---|---|
| `Ph1` | **Plan & Draft** (entry rung) | 0a (`DETERMINISTIC_CHECKS.md` mandatory subset) + Grounding Rules 1, 5, 7a on the diff-scoped changed paragraphs. Generator drafts under the declared P-stage register. **No Self-Ph1 Verdict is emitted** — Phase 3.5 retired at v0.7.0. Approval writes `ph1_draft_completion_signed` then `user_approval` and auto-advances section to **Ph2 entry**. | Planner + Generator + Reflector-lightweight. **Evaluator NOT engaged** at Ph1. | Approval bumps `current_phase → Ph2`; if `section_ceiling_override = Ph1` the section locks (`ceiling_locked`) and further `/review` invocations refuse-to-escalate. | No |
