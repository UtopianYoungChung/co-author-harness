...
### 3.3 By tier (v0.7.0 — Lifecycle-Stage Ladder)

The Planner reads the `tier:` field from `reviews/classification.md` and the per-section state in `reviews/tier_state.json` to choose the dispatched rung. `tier:` at v0.7.0 takes legal values from `{T1, T2, T3, T3_converged, T4, ceiling_locked}` (the `T0` state probe is retired since v0.5.5; the v0.6.0 `T4_ready` value is renamed to `T3_converged` at v0.7.0) with `T3R` as an independent sibling ladder for response letters. The four lifecycle stages are: **T1 Plan & Draft**, **T2 Review & Revise**, **T3 Iterate & Converge**, **T4 Finalize & Close**. Each rung runs `plan → (review at T2+) → generate → human approval`; user approval auto-advances the section to the next rung (or locks its ceiling if the ceiling was set below the default). For the full ladder semantics — rung preconditions, the binary Approve / Reject gate, the Manuscript Convergence Report (MCR), and the fifteen-field `tier_state.json` invariant — see `references/TIER_PROTOCOL.md §§2–6`.

| Tier | Name | Required review steps at this rung | Agent engagement | Ceiling effect | G.4 sign-off |
|---|---|---|---|---|---|
| `T1` | **Plan & Draft** (entry rung) | 0a (`DETERMINISTIC_CHECKS.md` mandatory subset) + Grounding Rules 1, 5, 7a on the diff-scoped changed paragraphs. **When `sd_sr_required: true` in `reviews/classification.md`** (v0.7.1 opt-in — `TIER_PROTOCOL.md §3.1.2`; default `false`), the Planner also authors i* SD/SR models and runs the §3.1.1 structural-completeness validator; otherwise the SD/SR authoring step is out of scope. Generator drafts under the declared P-stage register. **No Self-T1 Verdict is emitted** — Phase 3.5 retired at v0.7.0. Approval writes `t1_draft_completion_signed` then `user_approval` and auto-advances section to **T2 entry**. | Planner + Generator + Reflector-lightweight. **Evaluator NOT engaged** at T1. | Approval bumps `current_tier → T2`; if `section_ceiling_override = T1` the section locks at `T1_ready` (ceiling_locked) and further `/review` invocations refuse-to-escalate. | No |
| `T2` | **Review & Revise** (first Evaluator engagement) | 0a + Step 2 (playbook) or Step 3 (Baird, if IS) scoped to the section envelope + Step 7 (integrated checklist, local items only) + SAFEGUARD checks 1, 4, 5. **Confirmation Mode at T2 entry is retired at v0.7.0**: every T2 entry runs a fresh Evaluator local-scope pass. Pre-tier-advance check clauses (a)(b)(d)(g) always enforce: T1 exit artefact presence, 15-field section populated, non-null `t1_pstage_declaration` (E-PSTAGE-REQUIRED-AT-T2), row-shape conformance. **Clause (e) (i* validator passed / SD groundable — E-IMODEL-STRUCTURALLY-INCOMPLETE and E-T2-SD-UNGROUNDABLE) runs only when `sd_sr_required: true`** (v0.7.1 opt-in — `TIER_PROTOCOL.md §3.1.2`); when the flag is absent or `false`, SD/SR checks are skipped silently. Emits `reviews/local_findings_<date>.md` tagged `scope: section`. EG-3 fires on any cross-scope reference ("the finding or fix references material outside the section's `heading_path`"). | Full four-agent loop with Reflector-lightweight. | Approval bumps `current_tier → T3` and sets `t3_last_activity_at`; ceiling-locked at `T2_ready` if `section_ceiling_override ≤ T2`. | No |
| `T3` | **Iterate & Converge** (unbounded loop) | 0a (incl. §9a, §9b pre-filters) + 0b + Steps 1–7 + Step 8 (synthesis) + Step 8.5 (all eight SAFEGUARD checks, Coupling E.2 overlay optional). `convergence_metric = diff_lines_vs_previous_round / total_section_lines` computed per round; two consecutive rounds < 0.03 emits `[CONVERGENCE-STABLE]` (W-CONVERGENCE-STABLE) and flips `current_tier: T3 → T3_converged` via TerminalSignoffRow. `[T3-STALE]` is purely computed from `t3_last_activity_at` against wall-clock (Option A, no persisted flag); null short-circuits to false. Drift exceedance under tolerant mode at T3 emits `W-T3-DRIFT-EXCEEDED-TOLERANT` (warning only, not forced re-review — preserves T3's unbounded-loop contract). Linear-Accountability: ESCALATED findings require named owner; transfer requires `transfer_rationale` (E-OWNERSHIP-TRANSFER-WITHOUT-RATIONALE). | Full four-agent loop with Reflector-lightweight. | Two-round convergence flips `current_tier: T3 → T3_converged`; ceiling-locked at `T3_ready` if `section_ceiling_override ≤ T3`. | Optional |
| `T3R` | **Response-letter sibling** (independent entry via `/review-letter`) | `response-letter-review` seven checks + SAFEGUARD 1/4/5 + Rule 7a when Class-1 verifiers cited. Emits `reviews/response_letter_findings_<date>.md`, `reviews/response_letter_reframe_brief_<date>.md`, `manuscript/response_letter.md`. Escalation-out rules per `TIER_PROTOCOL.md §2.5` (the `t3r_escalation_out` trigger repurposes `mcr_admission` under the LCR→MCR rename). | Full four-agent loop with Reflector-lightweight. | Does not participate in the T1→T4 ladder; approval closes the response-letter cycle. | Optional (required if paired with a T4 manuscript pass) |
| `T4` | **Finalize & Close** (submission-bound, terminal composition) | 0a (incl. §9a, §9b pre-filters) + 0b + Steps 1–7 + Step 8 + Step 8.5 (all eight checks, Coupling E.2 overlay at ≥15% echo+shallow threshold) + Step 9 (re-check). External verifiers REQUIRED (Zotero, Scholar Gateway). **Admission gated by the Manuscript Convergence Report (MCR)**: T4 cannot open unless every section in `tier_state.json` reports `current_tier = T3_converged` or `ceiling_locked` AND no computed `[T3-STALE]`. `[T3-STALE]` block → E-MCR-BLOCKED-T3-STALE; user clears via `/t3-reengage`. Legal T4→T3 demotions: EG-1 (`eg1_t4_downgrade_to_t3`) on Rule 1–7 grounding violation; EG-7 (`eg7_mcr_readmission_after_class_change`) on classification change. | Full four-agent loop with **Reflector-FULL** (five-phase close-out: Phase 2b NEW-H-4 aggregated confirmation-failed history audit, Phase 3 lessons, Phase 4 skill proposals, Phase 5 memory; invokes SK-14 Coupling C wiki synthesis and SK-16 Coupling D M5 wiki ingest). | Approval sets `terminal_tier_reached = true`; final `user_approval` row has `new_tier: "T4"`. | **Required** (MASTER §G.4 table completed, including 8.5 row) |

**Diff-scoped vs. full-document reads.** At `T1` the Evaluator is **not engaged** (the Rule 1 tier-gated digest exception — see `GROUNDING_PROTOCOL.md §Rule 1` — applies to Planner-authored deterministic pre-flight on diff-scoped paragraphs at T1 only). At `T2` the Evaluator reads the section envelope on full-file reads — the digest exception **does not apply** at T2. At `T3`, `T3R`, and `T4` the Evaluator reads the full manuscript.
