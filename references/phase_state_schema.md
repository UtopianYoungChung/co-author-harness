...
**Reader-accessibility policy binding.** Optional `milestone_framework.policy_bindings.reader_accessibility` stores the canonical package profile path/hash, resolved project profile path/hash, all contributing package/project path/hash pairs (including the mandatory model-prose corpus), and live G/H/VE transition state. Transition meanings live in the profile; only counters, state, and event pointers live here. `classification.md`, normative prose, and calibration reports are evidence/generated views, never live transition authorities. M1 records intended readers; M3 binds the resolved profile; M4/M5 bind current manuscript hash, profile hash, Check 8 evidence path/hash, aggregate verdict, and phase. Drift or missing links emit `MF-POLICY`.
