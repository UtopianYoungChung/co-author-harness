...
These commitments are mutually reinforcing but *separable*. A piece can satisfy C-1 and C-2 while genuinely needing to violate C-3 (e.g., a methods note that is structurally a list, not an arc). **C-7 is the protective counterpart to the convergent commitments**: where C-1…C-4 say what the prose should move *toward*, C-7 names what the prose must not *lose*. The two can be in tension — a strongly Suchman (C-1) rewrite can flatten an author whose natural voice is not first-person — and that tension is resolved by surfacing to the user (§6), not by silent default. C-5 is the **meta-rule that operationalises** C-1…C-4: it is binding at every P-stage and every tier rung (T2 onward), and it is the only commitment that cannot be suspended via the §4 relaxation procedure — accessibility is Hard Constraint #8 of the Ph.D.-root CLAUDE.md and inherits the binding force of §9 (non-negotiable rules). The Planner names which of C-1…C-4, C-6, and C-7 apply to the current piece at classification time; the Generator and Evaluator hold the piece against *those* plus C-5 always.
...
- **Performative neutrality.** The Evaluator may not flag a violation of C-1…C-4, C-6, or C-7 with language that implies the rule is neutral hygiene (e.g., "the prose is bad" rather than "C-2 sentence-rhythm violation," "this metaphor is confusing" rather than "C-6 scoped-metaphor violation," or "this sentence is too long / too repetitive" rather than "C-7: this is the author's baseline idiolect, not a defect"). The commitment must be named so the author can dispute it.
