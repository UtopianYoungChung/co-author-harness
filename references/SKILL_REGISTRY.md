...
- **Created:** 2026-04-16
- **Source:** Synergy analysis 2026-04-16 — graphify produces 43 nodes / 53 edges / 7 communities / 81% EXTRACTED 19% INFERRED / section-level provenance at `LLM wiki/graphify-out/`, but zero file in `.paper-package/` references it (grep confirmed). SK-20 is the v0.3.0 minimum-viable pilot: a single Evaluator pre-flight hook that converts graph topology into findings in the pipeline's native output format, preserving uncertainty inheritance through graph-specific source tags. Roadmapped successors: SK-19 `graph-read-at-planner` (Coupling E.1, M1-timed planner input) and SK-21 `graph-contradiction-sweep` (Coupling E.3, extends check-contradictions across the corpus) — both deferred pending SK-20 pilot outcomes.
- **Tier:** Package
...
- **Coupling family:** Coupling E has three planned sub-couplings (E.1 planner-input, E.2 overlay-this-skill, E.3 contradiction-sweep). Only E.2 is active in v0.3.0; E.1 and E.3 are proposed but not yet built — deliberate scoping to validate the coupling mechanism before scaling.
...
- **Pattern:** Single-command entry point for the **T2 Review & Revise** rung of the Lifecycle-Stage Ladder — the Evaluator's first engagement rung (absorbing M4a activities). Runs 0a (`DETERMINISTIC_CHECKS.md` mandatory subset) + Step 2 (playbook) or Step 3 (Baird, IS only) scoped to the section envelope + Step 7 (integrated checklist, local items only) + SAFEGUARD checks 1, 4, 5. **When `sd_sr_required: true` in `reviews/classification.md`** (v0.7.1 opt-in — `TIER_PROTOCOL.md §3.1.2`; default `false`), the Evaluator's pre-entry read-prerequisites include the Strategic Dependency / Strategic Rationale (SD/SR) artefacts declared by `reviews/classification.md`; missing artefacts fire the `E-T2-SD-UNGROUNDABLE` finding class. When the flag is absent or `false`, SD/SR read-prerequisites are skipped and `E-T2-SD-UNGROUNDABLE` is not emitted. The retired Confirmation Mode is no longer entered at T2 or any rung; the retired EG-2 Self-T1 Verdict mismatch gate is also no longer applied. Full-file reads at the section envelope (plus all cited material) are the universal floor — the retired Rule 1 tier-gated digest exception is not invoked at any rung at v0.7.0. On user approval, the Planner advances the section to T3 (`prev_tier: T2`, `new_tier: T3`, `trigger: user_approval`, `actor: planner`). EG-3 fires as a warn-only out-of-scope advisory (EG-6 degraded to non-blocking at v0.7.0) on findings or fixes that reference material outside the section's `heading_path`.
- **Created:** 2026-04-19 (v0.6.0 Phase 6 skill build); rewritten 2026-04-20 (v0.7.0).
- **Source:** The v0.5.5 T2 "local" tier had no standalone skill entry point — it was dispatched implicitly by the Planner from the `tier:` field. The v0.6.0 staircase promoted T2 to a first-class approval rung between Draft and Verify; the v0.7.0 Lifecycle-Stage Ladder retains T2 as a first-class rung but retires Confirmation Mode entirely, removes the Self-T1 Verdict coupling, and introduces SD/SR read-prerequisites and the `E-T2-SD-UNGROUNDABLE` finding class. v0.7.1 gates the SD/SR read-prerequisite and `E-T2-SD-UNGROUNDABLE` on the opt-in `sd_sr_required` flag in `reviews/classification.md` (default `false`) — see `TIER_PROTOCOL.md §3.1.2`.
- **Tier:** Package (Cowork-installable .skill file)
- **Status:** Active
...
- **Status:** Active (v0.10.0+)
- **Depends on:** `references/GROUNDING_PROTOCOL.md` (Rule 4 quote-before-attribute, Rule 6 no gap-filling, Rule 7a chain-of-verification per admit), `references/EXTERNAL_VERIFIERS.md §§1.5, 2, 3` (named Ph2 operationaliser of the wiki-first ladder per architecture §5.6), `references/PHASE_PROTOCOL.md §3.2` (Ph2 charter; admission grammar — SK-NEW-C is advisory, not gating), `references/phase_state_schema.md §5` (atomic-write convention for REFERENCES.md), `reviews/classification.md` (`per_seed_cap_extend_snowball`, `max_iterations_extend_snowball` parameters), `references/REFERENCES.md` (anchor seed pool — must be non-empty), Class 1 verifiers (Scholar Gateway primary; Zotero+Scite for Class 2/3 resolution; Consensus when registered), `agents/planner.md §Phase 3.8` (canonical Ph2 dispatch contract; halt-vs-continue authoritative)
- **Sibling:** SK-33 `seed-snowball-discovery` (upstream — SK-NEW-C extends the pool SK-33 produced); SK-34 `claim-coverage-audit` (upstream trigger — SK-34's BELOW_THRESHOLD verdict surfaces the uncovered claims SK-NEW-C resolves; the audit's `## Uncovered` table is SK-NEW-C's input queue when auto-dispatched from `run-phase-2` Step 0.5); SK-12 `grounding-audit` (adjacent, not equivalent — SK-12 audits claim-source coupling at the rhetorical level after the pool is curated; SK-NEW-C extends the pool itself); SK-NEW-D `inherit-snowball-from-wiki` (orthogonal, S6 deliverable — operates at Ph1 pre-seed time, not Ph2 in-loop)
- **Not a replacement for:** SK-33 `seed-snowball-discovery` (SK-33 is saturation-focused with `rate < ε` stop rule across the section claim register; SK-NEW-C is resolution-focused with single-claim early-break — different stop rules by construction); SK-34 `claim-coverage-audit` (SK-34 measures the coverage gap; SK-NEW-C closes individual gaps one claim at a time); the Evaluator's Step 4 judgment (SK-NEW-C surfaces *candidate* sources via lexical / claim-kind resolution; the Evaluator adjudicates whether each candidate resolves the claim under domain judgment per Rule 7a); a Class 2 / 3 verifier (SK-NEW-C invokes Class 1 verifiers only; Class 2/3 fallback is at the Evaluator's discretion when a `[BLOCKER]` finding lands and the user accepts grey literature per architecture §7 R-3 mitigation)
