...
- **Coupling family:** Coupling E has three sub-couplings (E.1 snowball-seeding, E.2 overlay-this-skill, E.3 contradiction-sweep). E.1 and E.2 are active at v0.10.0: E.1 is materialised via **SK-33 `seed-snowball-discovery`**'s graph-substrate iterate phase (previously roadmapped as `SK-19 graph-read-at-planner`, now retired; implemented at v0.10.0-S1.5); E.2 is this skill. E.3 (`SK-21 graph-contradiction-sweep`) is proposed but not yet built.
...
- **File:** `skills/inherit-snowball-from-wiki/SKILL.md` *(not yet implemented — S6 deliverable; see architecture §6.8)*
...
- **Created:** 2026-04-27 (forward reference — not yet active)
- **Source:** v0.10.0 Stage S6 deliverable per `docs/superpowers/plans/2026-04-26-snowball-reference-architecture.md §§5.5.5, 6.8` and `docs/superpowers/plans/2026-04-26-snowball-implementation-strategy.md §5.6`
- **Tier:** Package (executor — Sonnet; S6 to confirm)
- **Status:** Forward reference — not yet active (S6 target)
- **Depends on:** *(to be specified at S6)* `references/EXTERNAL_VERIFIERS.md §1.5`, `reviews/classification.md` (`inherit_snowball`, `pre_seed_cap` parameters), `${wiki_path}/graphify-out/graph.json` + `GRAPH_REPORT.md` (pre-populated community graph from a prior project's run)
