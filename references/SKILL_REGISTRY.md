...
- **Tier:** Package
- **Status:** Active
- **Depends on:** `references/PHASE_PROTOCOL.md §3 (Ph1 Plan & Draft)`, `references/GROUNDING_PROTOCOL.md` (full-file reads at every rung — no digest exception at v0.7.0), the per-section ledger `reviews/phase_state.json` (18-field SectionStateObject including `t1_pstage_declaration`), and the Planner's `tier_entry_log` six-field row contract per `AGENT_ORCHESTRATION.md §8.2a` (`prev_tier`, `new_tier`, `trigger`, `actor`, `notes`, `timestamp`).
...
- **Status:** Active
- **Depends on:** `references/PHASE_PROTOCOL.md §3 (Ph3 Iterate & Converge)` and `§6 (Manuscript Convergence Report)`, `references/REVIEW_ORCHESTRATION.md §3.3` (tier table), the full seven-step review references, the `reviews/phase_state.json` ledger (18-field SectionStateObject including `convergence_metric`, `t3_last_activity_at`), `reviews/convergence_log.md`, and `reviews/classification.md` declaring `tier: T3` (or the section's current_tier reaching T3 through ladder advance).
