...
**Lifecycle axes.** Per-section advancement uses the **Lifecycle-Phase Ladder** — **Ph1 Plan & Draft → Ph2 Review & Revise → Ph3 Iterate & Converge → Ph4 Finalize & Close** — defined in `PHASE_PROTOCOL.md` and recorded in `reviews/phase_state.json`. The unsplit M1–M5 milestone axis is orthogonal: milestones govern accepted dependency-bearing deliverables and F9 handoffs under `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md`; phases govern revision/readiness. Neither axis is absorbed into or renamed by the other. Ph4 admission remains MCR-gated: every in-scope section must satisfy the convergence/ceiling clauses and no section may be stale. Agent engagement is phase-conditioned: Evaluator dormant at Ph1, joins at Ph2, full four-agent loop at Ph3/Ph4. The four-agent contract and Grounding Protocol are unchanged.
