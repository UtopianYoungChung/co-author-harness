...
Projects do not self-declare exemplar status. A harness or portfolio registry grants `clean_lifecycle_exemplar` or `legacy_migration_exemplar` only after class-specific evidence and validator results are independently checked.

A clean exemplar requires native M1-M5 handoffs, a validator-clean unified ledger and generated view, current-hash M5 manuscript/export approval, a full release-gate pass, and at least one independent replay. A legacy exemplar requires an explicit legacy label, frozen raw evidence, separate lineages, an approved migration with all holds resolved, idempotence, rollback verification, and clean phase and milestone validation. Registry evidence records class, project path, approval authority, validator version, evidence paths, registration time, and current ledger hash.
