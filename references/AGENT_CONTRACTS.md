...
- `reviews/mcr_<YYYY-MM-DD>.md` — Manuscript Convergence Report (written when `/run-finalize` is invoked against a non-uniform ledger).
...
- `reviews/migration_report_v060_to_v070.md` — written once by migration script; Planner presents to user before accepting the first lifecycle review invocation on a migrated project (read-and-present, not write).
...
- The active deliverable named by `role_output_contract.json` and prior `manuscript/revision_log.md` entries.

**Outputs (author through the scoped writer transaction).**
- M1: `milestones/M1_project_memo.md` in Ph1.
- M2: `milestones/M2_annotated_references.md` in Ph1.
- M3: `milestones/M3_argument_evidence_outline.md` in Ph1, structured outline only.
- M4: `milestones/M4_complete_paper_draft.md` in Ph1–Ph3; initial assembly in Ph1 and finding-driven revision from Ph2.
- Public FINAL / ledger M5: `milestones/M5_final_paper.md` and its released export `submission_bundle/final_manuscript.md` in Ph4.
...
| Did a `full_lifecycle` round produce F1 / F2 / F3 / F5 artefacts without a corresponding F6 dispatch-plan artefact present (I-Planner-10 / P-1 plan-presence rule)? | Planner | Per-cycle `reviews/dispatch_plan_<cycle_id>.md` existence check; absence with any downstream artefact in the same `full_lifecycle` cycle is `R-Refl-DP-2` BLOCKER. `lab_iteration` must not emit F6; missing F6 there is not `R-Refl-DP-2`. An F6 path present under `lab_iteration` is `R-Refl-DP-LAB-FORBIDDEN` BLOCKER. |
