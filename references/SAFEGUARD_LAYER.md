...
3. **Audit positive markers (presence signals compliance).** Within each non-technical passage, check three positive markers: (a) **concrete-referent anchoring** — at least one concrete noun, analogy, or scenario per paragraph (abstract nominalisation chains without an anchor are flagged); (b) **agent-verb-object default** — majority of sentences take a human or identifiable agent as grammatical subject (nominalised constructions and passives that obscure agency are tracked); (c) **discourse-connective transparency** — transition words drawn from common English connectives (`but`, `so`, `because`, `this means`, `in other words`) rather than Latinate academic connectives (`notwithstanding`, `hitherto`, `whereby`) unless the Latinate form is genuinely more precise. Advisory-period default: qualitative judgment by the Evaluator; quantitative thresholds deferred to v0.10.2 retirement decision.
