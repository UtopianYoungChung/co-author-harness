...
| `coupling_c_active` | Legacy opt-in for the full Reflector's Phase 3 SK-14 handoff (default: same value as `wiki_linked`). Canonical Wiki mutation currently returns `WIKI_WRITE_TRANSACTION_UNAVAILABLE`; the field does not authorize a write. |
...
| C | Research → Wiki (lessons → syntheses) | Reflector-full Phase 3; mutation currently deferred | `agents/reflector-closeout.md §Phase 3`, `AGENT_ORCHESTRATION.md §8.5`, `skills/promote-lessons-to-wiki/SKILL.md` | SK-14 |
