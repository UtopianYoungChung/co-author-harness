...
│   │   ├── assignment/                   [runtime] Immutable receipt state (`ready/`, `reserved/`, `consumed/`, `invalidated/`), scoped `staged/`, and transition `claims/`; never acceptance state
│   │   └── policies/
...
The strict gate has three outcomes. `READY` means SK-20 is enabled and every graph-overlay precondition is current. `NOT_APPLICABLE` means a higher-authority project record explicitly disables SK-20 and supplies the complete N/A record described in Step 5; it exits 0 and the evaluator continues without an overlay. `MISCONFIGURED` means applicability is absent, contradictory, malformed, or an enabled input is missing/stale; it exits 4 and blocks the evaluator pre-flight. A `sk20_noop_YYYY-MM-DD.json` file is therefore evidence of either authorized N/A or a configuration block—read its `outcome` field instead of treating every no-op as success.

`--strict-exit` is the operational mode used by `run-evaluator-preflight.ps1`. Without it, the command is observational: it emits the same `outcome` and evidence but retains exit 0 for semantic `MISCONFIGURED` results so diagnostics can be collected without controlling a workflow. Usage errors still exit 1, and I/O/parse/safety errors still exit 2 in either mode. Evidence writes use staged, flushed, atomic replacement inside a real non-reparse `reviews/` directory; project-root files, junctions, symlinks, and outside output/evidence paths are refused. Once the complete new evidence set is installed, backup cleanup is best-effort and any harmless residue is reported in the output `warnings` array rather than rolling back committed evidence. The v0.4.10 read interface remains intact: absolute `--manuscript-path`, `--references-path`, and `--classification-path` values may point to readable regular files outside the project and are never written; relative overrides retain their existing invocation-relative meaning.
...
| `sk20_not_applicable_authority` | Required when `wiki_linked` or `coupling_e_on_review` is `false`: one of `user`, `venue`, `advisor`, `instructor`, `committee`, or `project_local_contract`. |
| `sk20_not_applicable_reason` | Required non-empty rationale for the explicit N/A decision. Absence is `MISCONFIGURED`, not an implicit no-op. |
| `sk20_not_applicable_scope` | Required structured scope: exactly `SK-20`, `Coupling E.2`, or `Coupling E.2 / SK-20`. Unrelated prose is not authorization. |
| `sk20_not_applicable_substitute_evidence` | Required project-relative path to existing substitute evidence used by the evaluator when the overlay is omitted (normally `research_notes/directives.md`). Absolute paths and parent traversal are rejected. |
...
**When Coupling E.2 fires.** At every Evaluator pre-flight, at **Step 0 — the first Evaluator action, before Step 0a (deterministic checks) and before Step 1 (classification gating)**. The ordering is deliberate: the overlay is the gating probe that tells the Evaluator whether graph-grounding findings need to enter the manuscript read at all, and Step 0a's deterministic checks consume the overlay's source-tag output when grading citations. The pre-flight agent invokes **SK-20 `graph-grounding-overlay`** (`skills/graph-grounding-overlay/SKILL.md`, formalized 2026-04-16). SK-20 resolves `wiki_path` from project metadata (or explicit overrides), reads `${wiki_path}/graphify-out/graph.json` and `GRAPH_REPORT.md`, overlays them on the manuscript's citation set, and writes `reviews/graph_overlay_YYYY-MM-DD.md` containing up to three finding types (graph-stub citations, section-location mismatches, missing-citation candidates) — each tagged with confidence inherited from graphify's `EXTRACTED`/`INFERRED`/`AMBIGUOUS` labels. The overlay is **pre-flight input** to Step 0a and Step 1, not a replacement for any pipeline step; the Evaluator's subsequent seven-step judgment pass consumes the overlay's candidates alongside its own. An authorized `coupling_e_on_review: false` record yields `NOT_APPLICABLE` and the evaluator proceeds without overlay findings. Missing `graphify-out/`, invalid graph data, or a stale graph while SK-20 is enabled yields `MISCONFIGURED` and blocks pre-flight until repaired or explicitly disabled. The `skills/run-phase-3/SKILL.md` and `agents/evaluator.md` Step-0 sections are the runtime authority for this order; any document that describes a different ordering is stale.

**No-wiki fallback.** If `wiki_linked: false`, record the four `sk20_not_applicable_*` fields above. The project then proceeds normally under an auditable `NOT_APPLICABLE` result: none of the couplings fire, no SK-14/15/17/20 invocations occur, and the Reflector's Phase 3.5 is a no-op. A bare `wiki_linked: false` is incomplete configuration and exits 4. Later migration to a wiki-linked configuration can be done by flipping `wiki_linked: true`, setting `coupling_e_on_review: true`, retiring the N/A directive, running a retroactive Coupling C (on accumulated lessons), running Coupling D if M5 has been reached, and supplying current graphify outputs before the next evaluator round.
