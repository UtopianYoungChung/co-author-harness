...
| `sk20_not_applicable_scope` | Required non-empty scope of the exception (normally `Coupling E.2 / SK-20`). |
...
**Applicability precedence.** Command-line values supplied by the invoking user take precedence over `research_notes/directives.md`; recognized SK-20 fields in `research_notes/directives.md` take precedence over project `CLAUDE.md`; package defaults come last. The gate merges only declared fields, so a directive may specialize a project configuration. Once a higher-precedence directive begins an N/A record, however, missing authority, reason, scope, or substitute evidence is `MISCONFIGURED`; the gate never falls back to a lower-precedence implicit default. Boolean values are exactly `true` or `false`; strings such as `off`, `no`, and `0` are malformed rather than false-like aliases.
