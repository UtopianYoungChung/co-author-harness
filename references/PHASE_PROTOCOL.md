...
- **Ph1 Plan & Draft** — Planner + Generator + bounded Evaluator policy pass. Produce each M1-M4 draft under the centroid and complete applicable governing-style bundle, then independently evaluate the exact bytes before milestone record or approval. Exit artefact: user-signed `reviews/ph1_draft_completion.md`. M1→M2→M3 supply the planning foundation. The bounded pass does not replace the full Ph2 review. Reflector remains lightweight.
