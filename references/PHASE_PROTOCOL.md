...
- **Gate trigger.** On every `TerminalSignoffRow` write attempt (`is_terminal: true`), the Planner reads the canonical Check 8 JSON evidence bound by `milestone_framework.policy_bindings.reader_accessibility`. A recomputed A–H aggregate of `BLOCKER` refuses the write with **`E-Ph3-ACCESSIBILITY-BLOCKER-AT-SIGNOFF`** (see `phase_state_schema.md §6.1). Transition meanings and required counts come only from profile keys `transitions.G` and `transitions.H`; live counters and append-only Planner approval events come only from the matching policy-binding transition objects. Classification dates and prose never activate or retire a transition. VE remains outside the aggregate in every transition state.
