...
**Typed output boundary.** This file governs phase behavior; `references/role_output_contract.json` 3.0.0 governs the six fixed roles and nine triggered F1-F9 output classes. Phase labels, terminal states, file presence, and readable legacy artifacts do not by themselves close an occurrence or prove shipment-v2 application or acceptance.
...
The Planner's round-entry responsibilities at v0.7.4 include authoring an F6 `planner_dispatch_plan` artefact at Phase 0.6 of every `full_lifecycle` round under invariant I-Planner-10 (`AGENT_CONTRACTS.md §2 Planner`). The artefact lives at `reviews/dispatch_plan_<cycle_id>.md`, conforms to `ARTEFACT_FRONTMATTER_SCHEMA.md §7a`, and is presented to the user as a blocking checkpoint — no downstream Evaluator, Generator, or Reflector dispatch fires until `user_approval_signature` is populated. The plan declares `sections_in_scope`, `dispatched_agents[]` (each with `agent`, `phase`, `model_allocation`, `scope`, `purpose`), `checks_scheduled[]`, and any `subagent_envelope[]` entries. The Planner's subsequent Phase 4.5 (model allocation) and Phase 4.6 (subagent envelope) become *consumers* of Phase 0.6's pre-authored plan rather than independent resolvers; plan drift between Phase 0.6 and execution is audited by Reflector Phase 2f at step 6.9 (R-Refl-DP-1 MAJOR, R-Refl-DP-2 BLOCKER on `full_lifecycle` only, R-Refl-DP-3 BLOCKER). F6 is never authored in `lab_iteration`; a shipment-lane laboratory plan is not F6. The F6 is strict-family, is NOT inherited under P-2 stability sub-mode, and does not itself consume iteration budget. See `agents/planner.md` Phase 0.6 for the authoring procedure.
...
Milestones govern deliverable dependencies; phases govern revision readiness. The pre-advance guard delegates milestone predicates to `milestone_framework_validate.validate_gate` using the canonical effective handoff policy. Ph1→Ph2 requires M1, M2, and M3 to compute `READY`, `LEGACY_READY`, or authorized `NOT_APPLICABLE`; effective `audited` additionally requires the consumed predecessor chain, while effective `derived` requires the exact `not_applicable` representation and treats any optional F9 as non-gating and non-consumed. Ph4 admission requires accepted/current/approved M4 plus the policy-correct handoff representation in addition to the MCR/section clauses. Ph4 terminal close is a distinct preflight requiring current-hash M5 approval, no reopened or `needs_revalidation` state anywhere M1–M4, the policy-correct terminal handoff representation, signed `reviews/G4_signoff.md`, and signed `reviews/ph4_ship_signoff.md`. Each signoff file carries exactly one anchored line `status: PASS`, `status: APPROVED`, or `status: SIGNED`; missing, negative, negated, duplicate, or contradictory statuses block. At terminal close, G.4 additionally binds `manuscript_path`, `manuscript_sha256`, `round_id`, Evaluator authority, current Check 8 hash, and `safeguard_status: CLEAN`; the ship signoff binds the same manuscript identity and round with user authority. Public `FINAL` maps to ledger M5 and closes only through `assignment_milestone_checkpoint.py accept --milestone FINAL --terminal-evidence ...`, which publishes both terminal fields state-last and emits F9 only when effective policy requires it or `--emit-f9` explicitly requests optional evidence. `MISCONFIGURED` always blocks. Reopening or hash drift blocks through stale dependencies but never auto-demotes a phase. Planner/user adjudication is required.
...
*Normative status.* This file is the single source of truth for phase semantics at v0.7.4. Other package files (`AGENT_ORCHESTRATION.md`, `REVIEW_ORCHESTRATION.md`, `GROUNDING_PROTOCOL.md`, `SKILL_REGISTRY.md`, `ROUTING_SPINE.md`, the four `run-phase-N` skills) name and apply phase concepts but do not redefine them. Any conflict between this file and another package file is resolved in favour of this file unless the other file is explicitly `GROUNDING_PROTOCOL.md` (which sits outside the precedence ladder; see project AGENTS.md §5).
