...
The Planner records a successful gate result with `--emit-receipt <path>`. The path must match `reviews/.harness/assignment/gate_receipt_<target>_<utc>.json`; the gate refuses to overwrite an existing receipt. A receipt is evidence for one Planner-to-Generator dispatch, not a milestone ledger or an acceptance record. Its strict schema is `references/schemas/assignment_gate_receipt.schema.json`, and its non-authoritative bootstrap example is `references/templates/assignment_gate_receipt.json`.

Each receipt binds the project name, stage, target, active lineage, exemplar-conditioning decision, exact assignment-contract, phase-state, and package-profile hashes, plus a digest of the gate's READY output. `python scripts/assignment_process_gate.py --project-root <project-root> --verify-receipt <receipt>` accepts only a current `ready` receipt, re-hashes all bound files, and re-runs the sole `validate()` predicate engine for the recorded target. Verification never updates milestone state.

Receipts are single-use. The Planner changes `status: ready` to `consumed` after the dispatch round ends, including an aborted round, and records the lifecycle time in `consumed_at`; an explicitly cancelled receipt uses `invalidated` with the same timestamp field. Verification reports `APG-RECEIPT-MISSING`, `APG-RECEIPT-STALE`, `APG-RECEIPT-CONSUMED`, or `APG-RECEIPT-INVALID` and exits non-zero. Hash drift requires a new gate run and a new receipt; an old receipt is never refreshed in place.
