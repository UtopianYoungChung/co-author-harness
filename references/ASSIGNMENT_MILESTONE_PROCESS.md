...
Every M1-M4 and FINAL dispatch requires centroid-conditioned generation and an independent centroid evaluation. Existing, missing, stub, staged, migrated, and reopened artifacts receive the same policy resolution. The assignment receipt records that exemplar conditioning is required; it never claims that conditioning happened. Only a v2 passage receipt plus exact product-assurance report proves execution. The centroid resolver's successful state is `binding_resolved`, explicitly not `ready` or `conditioned`.
