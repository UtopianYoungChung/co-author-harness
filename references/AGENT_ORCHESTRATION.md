...
The mapping coordinates two contracts rather than collapsing them. M1-M3 retain separate deliverable and handoff gates inside Ph1, where each approval advances only the milestone chain and does not exit Ph1. The Generator writes the exact M1-M4 and public FINAL/M5 deliverable bytes; the Planner records user/advisor feedback, approval, state, and policy-correct handoff representations. M4 remains the manuscript deliverable from Ph1 initial assembly through Ph2-Ph3 review and convergence. Public FINAL records M5 and certifies the exact `milestones/M5_final_paper.md` plus released export bytes at Ph4. Machine-readable authority: `role_output_contract.json`.

#### Native course-essay auto-walk

For the native `course-essay-four-milestones-v1` profile, `/run-draft` runs `assignment_milestone_checkpoint.py derive` and obeys its executable target/action result. During gather that is the first non-accepted milestone; after materials are in play Joseph may name any started M1-M4. The Planner dispatches one deliverable at a time through a single-use receipt:
...
4. After explicit current-byte approval only: Planner runs `assignment_milestone_checkpoint.py accept`. Effective `audited` publishes exact F9 first; effective `derived` accepts without F9 by default and may emit the same exact packet only when explicitly requested. In every case `phase_state.json` is state-last. The next invocation runs `derive`, then `begin`; audited consumes the predecessor F9, while derived validates accepted/approved/current predecessor state and never consumes optional F9.
5. At M3→M4: before the acceptance transaction, write `reviews/.harness/assignment/wiki_grounding_<round>.json` after the wiki-first pass and bind its exact path/hash in M3 `policy_evidence`, or bind an explicit user/advisor/instructor opt-out. Any audited or explicitly requested optional derived F9 packet must carry the same binding.
6. M4 first-start: require accepted M1-M3 plus current wiki evidence; after materials are in play, restage of started M4 does not re-impose that predecessor lock. The same Yu-surface/Dennett-argument-only split already used at M1-M3 remains binding.
7. FINAL: after accepted M4 and Ph4 admission, run `begin --milestone FINAL`, then the final gate with still-current wiki evidence; emit and preflight a FINAL receipt for the contract-owned final manuscript and released-export paths. After scoped publication, run public `record --milestone FINAL`, stop for explicit current-byte approval, then run `accept --milestone FINAL --terminal-evidence <path>`. That single close validates complete round-bound F7/F8, structured G.4 and user ship signoff, Reflector-full, deterministic findings, convergence, Check 8, and immutable consumed FINAL receipt/result evidence; it publishes any policy-authorized `M5_terminal.json` evidence first and terminal state last. The M4-onward exemplar envelope remains available.

The loop is checkpoint-driven orchestration, not unattended acceptance. A request for a complete paper cannot skip first-start of ungathered M1-M4. After materials are in play, restage of any started M1-M4 is legal; FINAL still requires four current accepted hashes. Producing a complete essay or `reviews/ph1_draft_completion.md` as a substitute for that ledger is an explicit refuse condition and protocol violation. Legacy mode emits `APG-SEQUENCE-LEGACY` and names required migration/acceptance work; it never infers acceptance or materials-in-play from files. Ph1-Ph4 continue to govern revision maturity independently of this assignment sequence.
