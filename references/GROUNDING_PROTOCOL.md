...
- **v0.7.0 onward.** No rule-digest is a Rule 1 satisfier. Every citation, at every tier, must come from a read the agent performed in the current session (or earlier in the session, provided the file has not been edited since). The `scripts/verify_rule_digest.py` and `scripts/build_rule_digest.py` utilities are retained in the package tree for archive purposes but are not invoked by any v0.7.0 skill, agent, or release-gate phase.
