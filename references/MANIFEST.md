...
| `policies/reader_accessibility.v1.json` + `schemas/reader_accessibility_profile.schema.json` + `schemas/reader_accessibility_candidates.schema.json` + `schemas/check8_evidence.schema.json` | Single machine-readable policy plus strict candidate and adjudicated-evidence contracts; hashes are stored externally | Every Ph2–Ph4 Check 8 dispatch, F1, and MF-POLICY validation |
