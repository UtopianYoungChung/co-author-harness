...
| `policies/draft_governance.v1.json` + `scripts/draft_governance.py` + `scripts/centroid_service.py` + `scripts/source_extract.py` + `scripts/canonical_bibliography.py` + `scripts/evidence_publication.py` + `scripts/product_assurance.py` + `scripts/c2_evidence_validation.py` + centroid/extract/bibliography/product schemas | Universal M1-M4/FINAL resolver and product verifier; separates binding resolution from passage execution; publishes canonical extraction and Wiki-derived bibliography evidence through a claimed, prepared, journaled, recoverable, marker-last transaction; validates exact locators and manuscript spans, cited-subset identities, typed passage use, Evaluator-reviewed conditioning influence, and centroid/member coverage; binds independent current-byte evaluation to the complete policy bundle | Before and after every academic draft or revision; required by milestone `record` and terminal full-run validation |
