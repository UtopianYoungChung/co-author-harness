...
| `policies/draft_governance.v1.json` + `semantics_manifest.v1.json` + `scripts/draft_governance.py` + `scripts/draft_evidence_verifier.py` + `scripts/schema_runtime_check.py` + `scripts/centroid_service.py` + `scripts/source_extract.py` + `scripts/canonical_bibliography.py` + `scripts/evidence_publication.py` + `scripts/product_assurance.py` + `scripts/c2_evidence_validation.py` + centroid/extract/bibliography/product/verifier schemas | Universal M1-M4/FINAL resolver and shared product verifier; separates binding resolution from passage execution; publishes canonical extraction, Wiki-derived bibliography evidence, and recomputed product assurance through claimed, prepared, journaled, recoverable, marker-last transactions; validates exact locators and manuscript spans, cited-subset identities, typed passage use, Evaluator-reviewed conditioning influence, centroid/member coverage, current candidate adjudications, the exact transitive verifier semantics manifest, and the required Draft 2020-12 runtime | Before and after every academic draft or revision; verifier transactions become lifecycle evidence only after the C5 consumer migration |
