...
Assignment writer transaction entry points are `scripts/assignment_process_gate.py` (READY emission and read-only verification), `scripts/assignment_dispatch_preflight.py` (Planner reservation), `scripts/assignment_writer_commit.py` (journaled live-path publication), `scripts/assignment_receipt_invalidate.py` (explicit cancellation), and `scripts/assignment_receipt_recover.py` (inspected stale-claim recovery). State and target coordination is centralized in `scripts/assignment_receipt_transaction.py`; Planner M1-M4 and public FINAL/M5 derivation, begin, record, approval/F9 acceptance, terminal close, and inspected recovery use the single public `scripts/assignment_milestone_checkpoint.py` surface backed by `scripts/assignment_milestone_transaction.py`.
...
| `policies/draft_governance.v1.json` + `scripts/draft_governance.py` + `scripts/centroid_service.py` | Universal M1-M4/FINAL policy resolver and verifier; artifact presence is not a waiver; binds centroid-conditioned generation and independent current-byte evaluation to the complete applicable D-STYLE, grammar/style, grounding, deterministic, SAFEGUARD, and overlay bundle | Before and after every academic draft or revision; required by milestone `record` and terminal full-run validation |
