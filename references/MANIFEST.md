...
|---|---|
| **Any academic draft, review, edit, critique, or refinement** | `FIVE_ENFORCEMENT_MEASURES.md` first (U1–U5 umbrella); then resolve `policies/draft_governance.v1.json`; derive centroid applicability from the authoritative reader binding; reader-profile v2 with `semantic_usage: not_invoked` omits `/centroid-pass`, while a governed semantic binding runs it for generation and independent evaluation; always produce the separate C6 scholarly evaluation; use `scripts/audit/run_all.py` only for diagnostic mechanics and `scripts/run_product_gate.py --mode governed-product --scholarly-evaluation <path>` with the applicable committed verifier transaction and current C6 evidence for lifecycle-eligible product evidence |
...
| **Running the registered Chung academic-voice pass** | `chung_academic_voice_guidelines.md`; `skills/chung-academic-voice-pass/SKILL.md` — on-demand or Evaluator evaluate fire-table; not a STYLE_COMMITMENTS C-n |
