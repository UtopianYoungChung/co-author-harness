...
**Release-history integration.** Historical release narratives live in
`CHANGELOG.md` and `docs/release-notes/`; current package identity remains
...
Assignment writer transaction entry points are `scripts/assignment_process_gate.py` (READY emission and read-only verification), `scripts/assignment_dispatch_preflight.py` (Planner reservation), `scripts/assignment_writer_commit.py` (journaled live-path publication plus hash-chained mutation row), `scripts/assignment_mutation_check.py` (live postimage verification), `scripts/assignment_receipt_invalidate.py` (explicit cancellation), and `scripts/assignment_receipt_recover.py` (inspected stale-claim recovery). State and target coordination, including the exact reservation-transition-bound dispatch authorization fact, is centralized in `scripts/assignment_receipt_transaction.py`; generation/evaluation dispatch authority, its marker-last hash-chained issuance ledger, and one-time consumption are centralized in `scripts/assignment_dispatch_claim.py`; mutation genesis, authorized legacy anchoring, accepted-prefix checks, and live-head checks are centralized in `scripts/assignment_mutation_anchor.py`. Planner M1-M4 and public FINAL/M5 derivation, begin, record, approval plus policy-correct handoff acceptance, terminal close, and inspected recovery use the single public `scripts/assignment_milestone_checkpoint.py` surface backed by `scripts/assignment_milestone_transaction.py`.
...
|---|---|
| **Ordinary standalone named pass, new draft, or manuscript revision** | `PROJECT_INDEPENDENT_WORKFLOW.md`; `GROUNDING_PROTOCOL.md`; actual named skill and selected rule files. Read-only checks need no project or graph. Draft/revision uses distinct native Generator, Evaluator and Reflector; task completion is separate from lifecycle. |
| **Governed lifecycle academic draft, review, edit, critique, or refinement** | `FIVE_ENFORCEMENT_MEASURES.md` first (U1–U5 umbrella); then resolve `policies/draft_governance.v1.json`; derive centroid applicability from the authoritative reader binding; reader-profile v2 with `semantic_usage: not_invoked` omits `/centroid-pass`, while a governed semantic binding runs it for generation and independent evaluation; always produce the separate C6 scholarly evaluation; use `scripts/audit/run_all.py` only for diagnostic mechanics and `scripts/run_product_gate.py --mode governed-product --scholarly-evaluation <path>` with the applicable committed verifier transaction and current C6 evidence for lifecycle-eligible product evidence |
| **Applying the five enforcement measures / section-level argument hygiene** | `FIVE_ENFORCEMENT_MEASURES.md` (U1 vocabulary, U2 novelty shape, U3 ordering, U4 gap-before-claim, U5 one job per sentence); then the C-n / Check 8 / craft file that sits under the fired umbrella |
| **Transient proposal-only laboratory iteration** | `FULL_RUN_CONTRACT.md` (`lab_iteration`); resolve the exact governed Workbench project, assignment contract, and matching staging/private-shipment output; no lifecycle, F9, terminal, promotion, release, research-authority, or dissemination action |
| **Per-section phase advancement (Ph1 → Ph4)** | `ASSIGNMENT_MILESTONE_PROCESS.md` (pre-draft source/function gate); `policies/phase_engagement.v1.json` (sole role-engagement matrix); `PHASE_PROTOCOL.md`; `phase_state_schema.md` (incl. §2.2 stage/profile shadow fields at PR-3b.1); `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md` (pre-transition milestone gates) |
| **Project lifecycle, milestones, feedback, approval, or handoffs** | `ASSIGNMENT_MILESTONE_PROCESS.md`; bound profile under `policies/`; assignment receipt/checkpoint/approval/terminal-evidence schemas; `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md`; `AGENT_ORCHESTRATION.md` §10; derive/begin/record/accept with `assignment_milestone_checkpoint.py` (public `FINAL` maps to M5), use the scoped writer transaction for deliverable bytes, bind the same consumed receipt/dispatch chain into a current qualified C6 scholarly evaluation, keep user approval separate, and use the matching explicit recovery command when required |
...
| `policies/phase_engagement.v1.json` + `retired/PHASE_ENGAGEMENT_RETIREMENT_LEDGER.md` | Sole machine-readable Ph1–Ph4 role-engagement matrix plus the only permitted historical location for superseded participation assertions | Every phase-conditioned role dispatch; enforced by `scripts/phase_engagement_check.py` |
| `ASSIGNMENT_MILESTONE_PROCESS.md` + `policies/course_essay_milestones.v1.json` + `schemas/assignment_gate_receipt.schema.json` + `templates/assignment_gate_receipt.json` + dispatch-claim/dispatch-consumption/host-attestation/mutation schemas and remaining assignment templates | Source-bound assignment intake; executable native M1→M4→FINAL checkpoint walk; immutable scoped-writer receipt; kernel-issued role-scoped one-time dispatch authority; host evidence bounded by a trusted adapter; recoverable publication; mutation genesis or externally authorized legacy anchor; explicit approval; converged current-policy binding; state-last policy-correct handoff/lifecycle transactions; distinct final manuscript/export and transactional terminal close | Before every drafting dispatch and milestone decision; `scripts/assignment_process_gate.py`, `scripts/assignment_dispatch_preflight.py`, `scripts/assignment_dispatch_claim.py`, `scripts/assignment_writer_commit.py`, `scripts/assignment_receipt_invalidate.py`, `scripts/assignment_receipt_recover.py`, and `scripts/assignment_mutation_anchor.py` own the transaction boundaries; `scripts/assignment_milestone_checkpoint.py` owns derive/begin/record/accept including FINAL/M5 close |
...
| `scripts/migrate_lab_iteration_derived_handoff.py` + `schemas/milestone_handoff_policy_migration_authority.schema.json` + `schemas/milestone_handoff_policy_migration_claim.schema.json` + `schemas/milestone_handoff_policy_migration_receipt.schema.json` + `schemas/milestone_handoff_policy_migration_rollback_manifest.schema.json` + `schemas/milestone_handoff_policy_migration_rollback_receipt.schema.json` | Dedicated exact-preimage, authority-receipted `lab-iteration-derived-handoff-v1` dry-run/apply/verify/rollback transaction; changes only milestone contract `1.0.0` to `1.1.0` plus explicit `derived`, preserves all other ledger semantics and external bytes, and supports exact state-last recovery | Only an explicitly authorized existing-project policy migration; never ordinary bootstrap, validation, laboratory iteration, or live-research inference |
...
| `policies/draft_governance.v1.json` + `semantics_manifest.v1.json` + `scripts/draft_governance.py` + `scripts/draft_evidence_verifier.py` + `scripts/scholarly_evaluation.py` + `scripts/scholarly_evaluation_binding.py` + `scripts/run_product_gate.py` + `scripts/schema_runtime_check.py` + `scripts/centroid_service.py` + `scripts/source_extract.py` + `scripts/canonical_bibliography.py` + `scripts/evidence_publication.py` + `scripts/product_assurance.py` + `scripts/c2_evidence_validation.py` + lifecycle/product-gate/scholarly/centroid/extract/bibliography/product/verifier schemas | Capability-triggered M1-M4/FINAL resolver, shared verifier, C6 scholarly-evidence replay, lifecycle binding, and governed product gate; reader-profile v2 omits unavailable graph obligations without weakening non-graph controls, while governed semantic bindings retain centroid/member coverage and passage-use verification; separates diagnostic mechanics, deterministic binding, and lifecycle-eligible product qualification | Before and after every academic draft or revision; milestone and terminal consumers accept only current-byte C6 and lifecycle bindings, and governed product qualification requires the verifier evidence applicable to the declared capability state plus separate qualified scholarly evidence |
| `schemas/common_scholarly_primitives.schema.json` + claim/evaluation/control/obligation schemas and their transaction scripts | Hash-pinned Draft 2020-12 primitives and lifecycle authorities for scholarly claims, evaluation binding, control-plane transitions, obligation adjudication, and phase engagement | Every scholarly lifecycle transaction; consumers pin both canonical `$id` and exact common-schema bytes and reject same-ID/different-byte substitutions |
| `schemas/product_assurance_detector_corpus.schema.json` + `schemas/product_assurance_detector_score.schema.json` + `scripts/product_assurance_detector_evaluation.py` + frozen detector fixtures | Labeled development/held-out corpus and deterministic confusion-matrix scorer for candidate detector rules | Product-assurance detector changes; held-out labels remain frozen and distinct from tuning evidence |
| `scripts/receipt_compaction.py` + `schemas/compact_receipt_index.schema.json` | Lossless, deterministic receipt compaction with immutable detail blobs, recursive fact projection, exact restoration, and reparse-safe archive containment | Package-local or governed-staging receipt sets only; hashes evidence integrity but does not establish signer authenticity |
| `scripts/analysis/fixture_runner.py` + `scripts/analysis/fixture_cache.py` + `scripts/analysis/code_census.py` | Authoritative full fixture registry, diagnostic non-authoritative quick tier, and explicit opt-in content-addressed cache assistance. Cache entries bind runner/census/helper/suite bytes, invocation, exact tested-input digests, interpreter, and safe host environment; malformed exact-key entries void the run and staged entries publish only after a full green stable run | Maintainer diagnostics and C9 timing only. Canonical fixture manifests and clean release qualification always use the full registry with cache off; partial tiers never write or void canonical evidence |
| `scripts/qualification_environment.py` + `scripts/release_qualification_controller.py` + `scripts/qualification_plane_topology.py` + runtime/archive probes + token policy/check + host transaction and schemas | Controlled environment; durable child journal/recovery; clean source pre/post product binding; exact five-plane topology before runtime suites; debt ratchet; marker-last fresh-task catalog, cache, provenance, and complete loaded-path attestation | Release qualification. Plane/cache equality is not shipment, startup loading, or `HOST_QUALIFIED` |
| `scripts/write_release_checksum.py` + `scripts/release_evidence_index.py` + `schemas/release_evidence_index.schema.json` + `scripts/update_version_manifests.py` | Canonical checksum; immutable five-plane evidence chain/report; command-driven manifest parity | Package/release evidence only; package clearance, shipment, and host qualification remain distinct |
| `scripts/protocol_conformance_smoketest.py` + `scripts/protocol_conformance_fixture_support.py` | Synthetic recorded M1-to-FINAL protocol exercise using production receipt, claim, verifier, mutation, milestone, handoff, and terminal authorities plus the committed miniature PDF | Release regression evidence only; labelled `protocol_conformance`, with no claim of human-quality semantic judgment or host-attested agent separation |
...
| File | Role |
|---|---|
| `FIVE_ENFORCEMENT_MEASURES.md` | Core argument-hygiene umbrella (U1–U5). C-1…C-8 and Check 8 remain in force under those five. Not a C-n. |
...
| `chung_academic_voice_guidelines.md` | Authorial-register overlay; `/chung-academic-voice-pass` or Evaluator evaluate fire-table |
