...
|---|---|
| **Any academic draft, review, edit, critique, or refinement** | Resolve `policies/draft_governance.v1.json`; run `scripts/draft_governance.py` and `/centroid-pass` for both generation and independent evaluation; use `scripts/audit/run_all.py` only for diagnostic mechanics and `scripts/run_product_gate.py --mode governed-product` with the committed evaluation verifier transaction for lifecycle-eligible product evidence |
...
| `policies/draft_governance.v1.json` + `semantics_manifest.v1.json` + `scripts/draft_governance.py` + `scripts/draft_evidence_verifier.py` + `scripts/run_product_gate.py` + `scripts/schema_runtime_check.py` + `scripts/centroid_service.py` + `scripts/source_extract.py` + `scripts/canonical_bibliography.py` + `scripts/evidence_publication.py` + `scripts/product_assurance.py` + `scripts/c2_evidence_validation.py` + lifecycle/product-gate/centroid/extract/bibliography/product/verifier schemas | Universal M1-M4/FINAL resolver, shared verifier, lifecycle binding, and governed product gate; separates diagnostic mechanics from lifecycle-eligible product qualification; publishes canonical extraction, Wiki-derived bibliography evidence, recomputed product assurance, verifier transactions, and typed run manifests through claimed, prepared, journaled, recoverable, marker-last transactions; validates exact locators and manuscript spans, cited-subset identities, typed passage use, Evaluator-reviewed conditioning influence, centroid/member coverage, current detector-version and fingerprint-bound candidate adjudications, one-time dispatch consumption, the exact transitive verifier semantics manifest, and the required Draft 2020-12 runtime | Before and after every academic draft or revision; milestone and terminal consumers accept only current-byte lifecycle bindings, and governed product qualification requires a committed evaluation verifier transaction |
| `scripts/runtime_plane_probe.py` + `schemas/runtime_plane_receipt.schema.json` | Typed source/archive/cache parity and portable runtime self-check receipt; separates exact, CRLF-only, semantic, missing, and foreign files; preserves version/provenance distinctions and blocks import- or policy-participating extras | Release qualification of source, unpacked archive, and loader-managed host planes; never substitutes for host attestation or canonical archive identity |
| `scripts/archive_runtime_probe.py` + `schemas/archive_runtime_receipt.schema.json` + `scripts/write_release_checksum.py` + `scripts/release_evidence_index.py` + `schemas/release_evidence_index.schema.json` + `scripts/update_version_manifests.py` | Safe archive inspection/extraction and isolated runtime qualification; canonical read-back checksum publication; immutable hash-bound package/release evidence indices and derived report; command-driven authoritative/marketplace version parity | v0.39+ release integration, package clearance, shipment, and host-qualification evidence; package-cleared, shipped, and host-qualified remain distinct states |
