...
|---|---|
| **Any academic draft, review, edit, critique, or refinement** | Resolve `policies/draft_governance.v1.json`; derive centroid applicability from the authoritative reader binding; reader-profile v2 with `semantic_usage: not_invoked` omits `/centroid-pass`, while a governed semantic binding runs it for generation and independent evaluation; always produce the separate C6 scholarly evaluation; use `scripts/audit/run_all.py` only for diagnostic mechanics and `scripts/run_product_gate.py --mode governed-product --scholarly-evaluation <path>` with the applicable committed verifier transaction and current C6 evidence for lifecycle-eligible product evidence |
...
| `scripts/runtime_plane_probe.py` + `schemas/runtime_plane_receipt.schema.json` | Cleared-ZIP/source/cache provenance transaction with safe archive-member equality, explicit CRLF policy, stable digest, four core suites, and typed missing/contaminated/qualified cache states | Release qualification of unpacked archive and loader-managed cache planes; cache equality never claims that Cowork loaded the package |
| `scripts/host_qualification_transaction.py` + `schemas/host_qualification_transaction.schema.json` | Immutable marker-last host receipt binding fresh task, registry row, installed manifest, cache comparison, cleared ZIP, core probe, and package-clearance input; preserves pending, functional-unbound, failed, and qualified states | Optional fresh Cowork qualification after package clearance; unavailable live host evidence remains `HOST_QUALIFICATION_PENDING` and never blocks package work |
| `scripts/archive_runtime_probe.py` + `schemas/archive_runtime_receipt.schema.json` + `scripts/write_release_checksum.py` + `scripts/release_evidence_index.py` + `schemas/release_evidence_index.schema.json` + `scripts/update_version_manifests.py` | Safe archive inspection/extraction and isolated runtime qualification; capability-aware canonical checksum publication with exclusive-create fallback and typed no-partial refusal; immutable hash-bound package/release evidence indices and derived report; command-driven authoritative/marketplace version parity | v0.39+ release integration, package clearance, shipment, and host-qualification evidence; package-cleared, shipped, and host-qualified remain distinct states |
