...
| Release | Highlights |
| --- | --- |
| **0.32.0** | Adds SK-47 `/centroid-pass` — an on-demand front door onto the single-centroid Eric-Yu domain-native register (`write`/`review`/`revise`/`all`) that runs the three derivations outside the automatic Ph1-Generator / Ph2–Ph4 Sub-check-H path and outside the M1–M3 milestone fence. Override-with-warning (`APG-EXEMPLAR-M4-FENCE-BYPASSED`), advisory dry-run by default with `--apply` opt-in, and a package-default binding fallback so it runs on any manuscript. Reads a resolved profile only — never re-pins, never writes `phase_state.json`, never gates a signoff; C-7 fence and Dennett argument-only rule preserved. Additive — no register-model or four-agent-contract change. |
