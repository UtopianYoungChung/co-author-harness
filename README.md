...
[![Version](https://img.shields.io/badge/Version-0.15.0-0366D6?logo=semver&logoColor=white)](.claude-plugin/plugin.json)
...
| Release | Highlights |
| --- | --- |
| **Unreleased** | PR-3b.4 compatibility landing: public ladder becomes `/run-draft` → `/run-iterate` → `/run-finalize`; former `/run-phase-2` routes to `/run-iterate --profile refine`, and former `/run-phase-3-stability` routes to `/run-iterate --profile stability`. Legacy commands remain shipped as compatibility routers. |
