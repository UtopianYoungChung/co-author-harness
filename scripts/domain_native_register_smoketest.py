...
def write_fixture(root: Path) -> tuple[Path, Path]:
    wiki = root / "wiki"; workspace = root / "workspace"; (wiki / "wiki/sources").mkdir(parents=True); (wiki / "raw/corpus").mkdir(parents=True); (workspace / "knowledge/LLM wiki/graphify-out").mkdir(parents=True)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        next(entry for entry in binding["register_provenance"]["provenance"] if entry["role"] != "graph_provenance_only")["sha256"]="0"*64
        result=milestone_validator.validate_document(project,milestone_fixture._phase_document(ledger))
        assert any(f.code=="MF-POLICY-PROVENANCE" for f in result.findings)
        ledger=milestone_fixture._materialize_native_project(project); binding=ledger["policy_bindings"]["reader_accessibility"]
