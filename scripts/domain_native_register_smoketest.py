...
def write_fixture(root: Path) -> tuple[Path, Path]:
    ...
    members = policy.load_profile()["domain_native_register"]["exemplar_members"]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for payload in malformed_graphs:
            ...
            try: policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT)
        ...
        for role in ("graph_provenance_only", "exemplar_source_page", "surface_warrant_pdf"):
            try: policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT,_snapshot_hook=churn(role))
            except policy.PolicyError as exc: assert "changed during resolution" in str(exc)
        ...
        try: policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT,_snapshot_hook=replace_graph)
        except policy.PolicyError as exc: assert "changed during resolution" in str(exc)
        ...
        try: policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT,_snapshot_hook=churn_before_final)
        except policy.PolicyError as exc: assert "changed during resolution" in str(exc)
        ...
        churn=policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT)
        ...
        assert policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT)["attestation_view_pin"] != pins[0]
        page=wiki/"wiki/sources/yu-1995-istar.md"; page.write_text("---\ngrounding_status: full-read - changed note\n---\n",encoding="utf-8")
        annotation=policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT)
        assert annotation["exemplar_view_pin"] == pins[1]
        pdf=wiki/"raw/corpus/yu-1995-istar.pdf"; pdf.write_bytes(b"changed")
        assert policy.resolve_domain_native_register(profile,wiki_root=wiki,workspace_root=workspace,harness_root=ROOT)["exemplar_view_pin"] != pins[1]
    ...
    assert len(live["seed_resolution_map"]) == 3 and len(live["unresolved_seed_ids"]) == 9 and live["warnings"]
