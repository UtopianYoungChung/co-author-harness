...
def build_candidate_artifact(project_root: Path, manuscript_path: Path, phase: str, cycle_id: str, resolved: dict[str,Any]) -> dict[str,Any]:
    ...
    surface_members=resolved.get("register_provenance",{}).get("surface_exemplar_members")
    if surface_members is None:
        surface_members=check8_h_prefilter.surface_register_exemplars(profile)
    h=check8_h_prefilter.analyse(text,manuscript_path,profile,phase=phase,register_class=resolved["passage_scope_class"],surface_warrant_members=surface_members)
    surface_keys=[item["source_key"] for item in surface_members]
    ...
    artifact={"schema_version":"reader_accessibility_candidates.v1","phase":phase,"cycle_id":cycle_id,"manuscript_path":manuscript_binding,"manuscript_sha256":hashlib.sha256(manuscript_path.read_bytes()).hexdigest(),"profile_path":resolved["profile_path"],"profile_sha256":resolved["profile_sha256"],"source_bindings":resolved["source_bindings"],"register_class":resolved["register_class"],"passage_scope_class":resolved["passage_scope_class"],"candidate_only":True,"evaluator_judgment_required":True,"sub_checks":{
      ...
      "H":{"applicable":phase in {"Ph2","Ph3","Ph4"},"deterministic_disposition":"candidate_probe","ph2_scope":"orienting_clause blocker-candidate plus advisory passage roles" if phase=="Ph2" else None,"positive_marker_audit_required":True,"surface_warrant_source_keys":surface_keys,"bundles":[{"locator":b.locator,"passage_role":b.passage_role,"role_confidence":b.role_confidence,"role_reason":b.role_reason,"binding_status":b.binding_status,"word_count":b.word_count,"candidate_status":"overlay_positive_marker_audit_required","negative_prefilter_fired":b.any_fired,"probes":{"nominalisation":b.nominalisation.__dict__,"prep_run":b.prep_run.__dict__,"hedging":b.hedging.__dict__}} for b in h]}}}
    if resolved.get("contract_version")=="2.0.0" and resolved.get("binding_kind")=="reader_profile" and resolved.get("semantic_usage")=="not_invoked":
        artifact["schema_version"]="reader_accessibility_candidates.v2"; artifact["semantic_usage"]="not_invoked"
    else:
        artifact["attestation_view_pin"]=resolved["attestation_view_pin"]; artifact["exemplar_view_pin"]=resolved["exemplar_view_pin"]
