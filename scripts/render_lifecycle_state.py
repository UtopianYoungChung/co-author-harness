...
DERIVED_FROM = f"{SOURCE_RELATIVE}#milestone_framework"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        stream.write(f"[BLOCKER] {exc.code} @ {VIEW_RELATIVE}: {exc}\n")
        return exc.exit_code
    except OSError as exc:
        sys.stdout.write(f"[BLOCKER] MF-DERIVED @ {VIEW_RELATIVE}: filesystem error: {exc}\n")
        return 4
