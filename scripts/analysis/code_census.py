...
def emit_matrix(report: dict) -> str:
    ...
    # This map is itself a tested input. Reading the execution manifest here
    # creates a cycle: refreshing the map invalidates the evidence it embeds.
    # Keep source discovery stable and validate execution evidence in parity().
    lines += [
        "", "## Fixture execution evidence", "",
        "Observed suite invocations live in `fixture_manifest.json`, written only",
        "by the authoritative full runner after a successful stable run. Run",
        ...
        "", f"Discovered suite files ({len(report['suite_universe'])}):", "",
    ]
    for fixture_file in sorted(report['suite_universe']):
        lines.append(f"- `{fixture_file}`")
    return "\n".join(lines)
