...
9. **Assignment milestone branch (native M1-M3).** After the target deliverable's feedback has been recorded and adjudicated under `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md`, stop for explicit user approval of that milestone. Only explicit approval authorizes the Planner to atomically write `accepted` state. Effective `audited` finalizes exact F9; effective `derived` records `not_applicable` by default and may emit the same exact packet only when explicitly requested, without consumption authority. Any M3 F9 that is emitted must bind current wiki-grounding evidence or an authorized opt-out. End the invocation after the transaction with `current_phase: Ph1`; do not write a Ph1 exit artefact and do not run a phase-advance check. The next `/run-draft` derives the successor target.
...
- **User approves M1, M2, or M3 → milestone-only.** Planner commits the accepted milestone transaction and its policy-correct handoff representation; `current_phase` stays `Ph1`. No `ph1_draft_completion_signed` row is written.
