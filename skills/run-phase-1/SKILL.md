...
0. **Assignment-process receipt gate and active-target derivation.** Read `references/ASSIGNMENT_MILESTONE_PROCESS.md` and the controlling brief in full; require a resolved `reviews/assignment_contract.json`. For a native course essay, derive the active target as the first non-`accepted` M1-M4 milestone in `reviews/phase_state.json`. Run `python scripts/assignment_process_gate.py --project-root <project-root> --stage draft --target-milestone <M1|M2|M3|M4> --emit-receipt reviews/.harness/assignment/gate_receipt_<target>_<utc>.json`. M1-M3 omit `--exemplar-conditioning`; M4 includes it only when the approved dispatch will use exemplars. Put the exact lines `assignment_gate_receipt: <path>` and `assignment_gate_target: <T>` in the dispatch brief, then run `python scripts/assignment_dispatch_preflight.py --project-root <project-root> --receipt <path> --expected-target <T>` immediately before Generator dispatch. Any non-zero result halts dispatch and all academic deliverable writing. `APG-SEQUENCE-LEGACY` requires migration/acceptance work, never synthesized acceptance. The assigned deliverable's function governs; the phase ladder does not redefine it.
...
5. **Generator dispatch.** Planner hands the Generator the revision plan, diff scope, declared P-stage, `assignment_gate_receipt`, and `assignment_gate_target`. The Planner reruns `assignment_dispatch_preflight.py` immediately before the Agent-tool call; Generator independently reruns it before any academic write. After success or abort, the Planner consumes the receipt; cancellation before Generator begins invalidates it. Generator drafts under `agents/generator.md` Phase 2 (Execute the Plan).
...
9. **Ph1 exit artefact authoring.** Planner writes `reviews/ph1_draft_completion.md` naming (i) the declared P-stage, (ii) the deterministic-check summary, (iii) the Rule 1 scope verdict, (iv) the Generator's revision log reference, and (v) any Reflector-lightweight findings. The user reviews and signs. Writing `ph1_draft_completion.md` for a complete essay while any of M1-M3 is non-`accepted` is a protocol violation; Ph1 maturity never authorizes a jump to M4.
10. **`pre_phase_advance_check.py` (Planner phase 5.5).** Runs the six-clause guardrail from `PHASE_PROTOCOL.md §7.3`: (a) exit-artefact presence and well-formedness, (b) 18-field `SectionStateObject` populated, (c) ESCALATED-owner linearity (no ESCALATEDs yet at Ph1, trivially passes), (d) `ph1_pstage_declaration` non-null for the Ph2 admission (checked at Ph2 entry; also surfaced at Ph1 exit as a warning), (f) MCR not applicable at Ph1 exit, (g) row-shape conformance of `phase_entry_log` rows per `phase_state_schema.md §3a.1`. (Clause (e), the i\* structural-completeness validator, was retired at v0.11.0.) Any failure blocks the advance.
...
12. **Assignment milestone checkpoint (native M1-M3).** After the target deliverable's feedback has been recorded and adjudicated under `MILESTONE_FEEDBACK_HANDOFF_PROTOCOL.md`, stop again for explicit user approval of that milestone. Only an explicit approval authorizes the Planner to finalize the F9 packet and atomically write `accepted` state. On M3, the M3→M4 packet must also bind current wiki-grounding evidence or an authorized opt-out. End the invocation after the transaction; the next `/run-draft` derives the successor target. Never loop through multiple acceptances in one unattended run.
