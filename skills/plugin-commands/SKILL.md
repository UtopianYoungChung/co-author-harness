...
| New section, ceiling T1 | `/run-phase-1` (alias: `/run-draft`) — Step 4.5 auto-invokes `/seed-snowball-discovery` if `references_initialized: false` |
...
| Heading/boundary/cross-section restructuring needed | `/run-iterate --profile structural` |
| Default iteration (profile governed by F6 dispatch plan; use `--profile deep` for pre-MCR parity) | `/run-iterate` (legacy `/run-phase-3` still resolves) |
...
| Final sign-off, MCR, external handoff | `/run-phase-4` (alias: `/run-finalize`) — only when ladder/MCR preconditions in project docs are met |
...
| `/run-phase-1` | T1 Plan & Draft — Planner bootstraps the 15-field section state; Generator drafts under the declared P-stage register. No Evaluator; no Self-T1 Verdict (retired at v0.7.0). | First rung for a new section or section whose ceiling is T1. |
...
| `/run-phase-3` | Legacy compatibility command for former Ph3 canonical iterate; routes to `/run-iterate` with F6-selected profile (unbounded loop, convergence_metric stability test, full four-agent loop + SAFEGUARD + Coupling E.2 overlay). | Older project automation or slash history only; prefer `/run-iterate` (or `--profile deep` for pre-MCR parity) in new guidance. |
...
| `/run-iterate` | Public iterate stage. Profiles: `refine`, `structural`, `deep`, and `stability`; legacy `/run-phase-2`, `/run-phase-3`, and `/run-phase-3-stability` route here. | Default post-draft review and revision stage; choose `refine` for first-pass/diff work, `deep` for pre-MCR safety-net work, and `stability` for byte-stable inheritance. |
