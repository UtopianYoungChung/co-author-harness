...
| `/run-phase-2` | T2 Review & Revise — first Evaluator-joined rung; full local-scope pass every entry (Confirmation Mode retired at v0.7.0). Pre-tier-advance check enforces non-null `t1_pstage_declaration`. | After a T1 approval when the section is climbing toward T3 or its ceiling. |
...
| `/run-iterate` | Alias for /run-phase-3 — Ph3 Iterate & Converge under the v0.15.0-pre stage × profile vocabulary. Both names resolve to the same canonical workflow. | Same use cases as `/run-phase-3`; new vocabulary surface (v0.15.0-pre PR-3b.3). Stability sub-mode still invoked via `/run-phase-3-stability` directly. |
