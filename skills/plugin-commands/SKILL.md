...
| `/run-phase-1` | Legacy compatibility route to public /run-draft; preserves the full Ph1 implementation body. Retains the 18-field section state contract. | Older project automation or slash history only; prefer `/run-draft` in new guidance. |
...
| `/run-phase-3` | Legacy compatibility route to public /run-iterate; preserves the full Ph3 implementation body. | Older project automation or slash history only; prefer `/run-iterate` in new guidance. |
...
| `/run-phase-4` | Legacy compatibility route to public /run-finalize; preserves the full Ph4 implementation body. | Older project automation or slash history only; prefer `/run-finalize` in new guidance. |
| `/run-draft` | Public draft-stage entrypoint; legacy /run-phase-1 remains a compatibility route. | Start or continue the draft-stage M1→M4 lifecycle. |
| `/run-iterate` | Public iterate stage — canonical post-draft iteration surface (profiles refine, structural, deep, stability); legacy /run-phase-2 and /run-phase-3-stability route here. | Post-draft review and revision; choose the profile required by the F6 dispatch plan. |
| `/run-finalize` | Public finalize-stage entrypoint; legacy /run-phase-4 remains a compatibility route. | Submission-bound close-out after MCR admission. |
| `/run-reflection` | Public reflection router; requires mode lightweight or full. | Use lightweight for Ph1-Ph3 integrity probes and full for Ph4/G.4 close-out. |
...
| `/repin-register` | Recompute the two domain-native register semantic pins through the package loader; optionally validate and stage one exact-key exemplar add/drop in that same confirmed epoch; record a package snapshot and append-only event; and optionally emit a Planner-owned project rebind request. Never rewrites `phase_state.json`, auto-promotes pending exemplars, or moves an open round's yardstick. | At milestone transitions, after grounded-source admission, after an MF-POLICY pin-stale discovery, or during explicit package maintenance. |
| `/centroid-pass` | Public centroid-pass invocation is unavailable; it returns a deterministic read-only IMPLEMENTATION_MISSING envelope and performs no writes. The separate repository analysis-packet service remains maintainer-only pending an H2 promotion decision. | Inspect the truthful public disposition; do not expect manuscript edits, milestone-fence bypass, semantic findings, or Wiki mutation. |
...
| `/graph-grounding-overlay` | **Unavailable** (`GRAPH_GOVERNED_GENERATION_UNAVAILABLE`); valid graph structure cannot activate the overlay. | Inspect the deterministic no-op only; no graph-derived findings or Wiki mutation. |
