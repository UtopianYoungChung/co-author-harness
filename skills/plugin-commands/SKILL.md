...
| `/repin-register` | Recompute the two domain-native register semantic pins through the package loader, record a package snapshot and append-only re-pin event, and optionally emit a Planner-owned project rebind request. Never rewrites `phase_state.json` or moves an open round's yardstick. | At milestone transitions, after grounded-source admission, after an MF-POLICY pin-stale discovery, or during explicit package maintenance. |
