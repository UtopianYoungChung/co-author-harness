...
**[CONVERGENCE-BLOCKED-ACCESSIBILITY].** If the line-diff metric is stable but Check 8 carries a live BLOCKER, the Planner emits `[CONVERGENCE-BLOCKED-ACCESSIBILITY]` instead of `[CONVERGENCE-STABLE]`, naming the open finding(s). The advisory blocks `/t3-terminate` — the write attempt returns `E-Ph3-ACCESSIBILITY-BLOCKER-AT-SIGNOFF` (`phase_state_schema.md §6.1`) and the section remains at `current_phase: Ph3`. Iteration is not charged against the budget for the refused write; the next Generator + Evaluator round that clears the BLOCKER *is* charged (`PHASE_PROTOCOL.md §3.3.3`). Check 8 carries eight Sub-checks A–H at v0.10.1 (was seven A–G prior); a Sub-check G BLOCKER under an active `advisory_until: next_manuscript_at_ph3` flag, or a Sub-check H BLOCKER under an active `advisory_until: H_two_revision_cycles` flag, is recorded in the aggregate but does not fire `[CONVERGENCE-BLOCKED-ACCESSIBILITY]` — the Planner reads the overlay's aggregate verdict directly and does not need separate flag-state arithmetic.
