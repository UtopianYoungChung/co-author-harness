...
3. Retrieve only passages admitted by the packet's member and warrant views.
   ...
   bytes, a non-empty extracted-passage file, its corpus/page locator, its
   `surface` or `argument` use, and the exact deterministic centroid packet.
