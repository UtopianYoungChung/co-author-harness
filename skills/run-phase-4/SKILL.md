...
cardinality, ordering, and typed suppression, before writing. Legacy readability
does not supply shipment-v2 transaction, application, or terminal authority.
`/run-finalize` is canonical; this file retains legacy
`/run-phase-4`. Reader-profile v2 runs graph-independent Ph4 and keeps the
semantic centroid dormant. Mechanical digests supply no semantic warrant;
semantic use requires authority.
