...
> **4.5 Planner: seed-snowball gate (conditional).** Read `classification.md` for `references_initialized: bool` (new field; defaults to `false` on fresh sections). If `false` or absent OR `references/REFERENCES.md` does not exist, dispatch SK-NEW-A `seed-snowball-discovery` for this section. The skill produces the populated REFERENCES.md and the snowball log. On clean exit, write `references_initialized: true` to `classification.md` and a `seed_snowball_signed` row (new trigger 31, see §6.3) to the section's `phase_entry_log`. On failure (Class 1 verifier unreachable + no wiki), the skill writes `[VERIFIER UNREACHABLE]` markers and the Planner emits a non-blocking `W-CORPUS-BOOTSTRAP-DEFERRED` warning; Ph1 proceeds with the `[UNVERIFIED]` floor and Ph2 entry will re-test.
...
- `references_initialized: bool` (default `false`) — set `true` after SK-NEW-A's clean exit at Ph1 Step 4.5. Read by SK-NEW-A's idempotency guard (skill exits no-op when `true`).
...
### 6.3 Stage S2 — Phase-1 wiring (Edit-1)

Insert Step 4.5 into `run-phase-1`. Add `references_initialized` to SectionStateObject and the 31st trigger (`seed_snowball_signed`). Migration script for existing `phase_state.json` files (sets the new field to `true` on sections that already have a populated REFERENCES.md, `false` otherwise). Risk: medium (modifies the Ph1 dispatch sequence; ledger schema bump). Validation: existing Ph1 sections do not break (idempotency guard fires); new sections trigger SK-NEW-A and the entry log shows the new row.
