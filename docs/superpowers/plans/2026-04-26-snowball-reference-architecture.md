...
> **0.5 Planner: claim-coverage audit dispatch (auto).** Dispatch SK-NEW-B `claim-coverage-audit` for the section. The skill reads `manuscript/<section>.md` and `references/REFERENCES.md` and produces `reviews/claim_coverage_<date>_<cycle_id>.md`. The Planner-side handler distinguishes **four outcome classes** — the canonical contract is anchored at `agents/planner.md §Phase 3.8`; the enumeration here is the index, not the spec (per §6.0 row 4 single-source-of-truth):
>
> - **(i) CLEAN** — `verdict: CLEAN`, coverage score ≥ `claim_coverage_threshold` (default 0.8). Planner writes `last_coverage_score`; no notification; no SK-NEW-C dispatch; Ph2 dispatch proceeds to Step 0a. The Evaluator's Step 8 synthesis still invokes Class 1 verifiers for BLOCKER citations as today.
> - **(ii) BELOW_THRESHOLD** — `verdict: BELOW_THRESHOLD`, coverage score < threshold. Planner writes `last_coverage_score`, emits non-blocking `W-COVERAGE-BELOW-THRESHOLD`, and auto-dispatches SK-NEW-C `extend-snowball-incremental` per uncovered claim, subject to the inline parallel-fanout cap (see closing paragraph below). The Ph2 Evaluator pass proceeds in parallel; if SK-NEW-C lands new sources before the Evaluator's Step 4 reaches the affected claim, the Evaluator reads against the extended pool. Otherwise the claim surfaces as a MAJOR finding in the Evaluator's report with `propose_extend_snowball` as the remediation hint.
> - **(iii) AUDIT_FAILED** — `status: noop` with one of the four non-degradable reason codes at `skills/claim-coverage-audit/SKILL.md §2` — `SECTION_AMBIGUOUS`, `EMPTY_SECTION_BODY`, `EMPTY_REFERENCES_POOL`, `CLASSIFICATION_MISSING` — or unrecovered execution error. Planner emits `E-COVERAGE-AUDIT-FAILED`, does NOT update `last_coverage_score`, does NOT emit `W-COVERAGE-BELOW-THRESHOLD`, does NOT auto-dispatch SK-NEW-C, and **proceeds to Phase 4 (Ph2 admission is NOT blocked).** The eight `pre_phase_advance_check.py` clauses remain the binding admission contract; the audit failure surfaces for user adjudication at cycle close, not for Ph2 entry refusal. This is the advisory-vs-substrate asymmetry against §5.2 Edit-1's Phase 3.7 outcome (iii) HALT — see §6.0 closing paragraph for the rationale.
> - **(iv) IDEMPOTENT_HIT** — `status: noop` with reason code `IDEMPOTENT_HIT` (manuscript hash AND `references/REFERENCES.md` hash unchanged from the prior cycle's audit). Treated identically to outcome (i) for outcome-handling purposes — no notification (neither `W-COVERAGE-BELOW-THRESHOLD` nor `E-COVERAGE-AUDIT-FAILED`); no SK-NEW-C dispatch; prior `last_coverage_score` remains current; prior `claim_coverage_<date>_<cycle_id>.md` remains the authoritative artefact for the Evaluator's reads. IDEMPOTENT_HIT is **excluded from outcome (iii)** explicitly — the asymmetric treatment (no notification on IDEMPOTENT_HIT vs. `E-COVERAGE-AUDIT-FAILED` on the four non-degradable codes) is the load-bearing rationale for the four-outcome enumeration rather than three.
...
**Two load-bearing details of the outcome (ii) handler are anchored at `agents/planner.md §Phase 3.8` and not restated here** (single-source-of-truth per §6.0 row 4):

- **Inline parallel-fanout cap of 8 SK-NEW-C invocations per Ph2 cycle.** Sourced from `max_parallel_extend_snowball` in `reviews/classification.md` (default 8 per the S4.5 R2 surfacing alongside the other coverage-tuning parameters; `extend_classification_md` populates the default during v0.9.0 → v0.10.0 migration). When the audit's uncovered-count exceeds the cap, the Planner dispatches the first 8 ranked by Rule-7a-criticality (claim-kind priority: result > mechanism > comparison > existential > theoretical commitment; ties broken by claim-locus order) and emits non-blocking `W-COVERAGE-FANOUT-CAPPED` naming the deferred claims; deferred claims surface as MAJOR Evaluator findings with the same `propose_extend_snowball` remediation hint as race-loser claims. Risk precedent: §7 R-1 (Class 1 verifier rate-limit) and R-12 (red-link cap); the SK-NEW-C dispatch fanout is a distinct cost surface and is now registered as **R-14** (added v0.10.1; see §7).
- **Cross-round coverage-regression hook consuming `last_coverage_score`.** Gated on `coverage_regression_floor` in `reviews/classification.md` (default 0.05 per the S4.5 R2 surfacing). On a non-IDEMPOTENT_HIT Ph2 re-entry where `prior_score - new_score > regression_floor`, the Planner emits non-blocking `W-COVERAGE-REGRESSION-OBSERVED` carrying `{section_heading_path, prior_score, new_score, regression, regression_floor, prior_audit_path}`. The hook is **non-gating**: a regression warning never refuses Ph2 admission, never blocks SK-NEW-C dispatch, and never overrides the eight `pre_phase_advance_check.py` clauses. On outcome (iii) the hook is skipped (no new score to compare); on outcome (iv) the hook is skipped (no round-over-round delta).
...
**Halt-vs-continue asymmetry — canonical example (added v0.10.1).** Row 4's "partial-failure halt-vs-continue semantics" admits the question of *when* a mid-run Step failure halts the cycle versus when it skips ledger writes and continues. The v0.10.0 release wired both polarities of this answer, with the asymmetry anchored at `agents/planner.md §Phase 3.7` and `§Phase 3.8` (single-source-of-truth per row 1) and surfaced here as the canonical example future stages should reason against:

- **Phase 3.7 outcome (iii) — HALT.** SK-NEW-A `seed-snowball-discovery` partial-failure modes (`WRITE_FAILURE`, `GRAPH_VS_EXTERNAL_DISAGREEMENT`, lock-contention; see §5.2 Edit-1 and `phase_notifications.yaml §4 E-SNOWBALL-MID-RUN-FAILURE`) **halt** the Ph1 cycle. Rationale: SK-NEW-A's output (the populated `references/REFERENCES.md`) is the *substrate* the Generator drafts against. A partially-populated REFERENCES.md is worse than no REFERENCES.md — the Generator would draft against an incomplete pool and the corruption would propagate into manuscript prose. Halt-and-adjudicate is the only safe move.
- **Phase 3.8 outcome (iii) — CONTINUE.** SK-NEW-B `claim-coverage-audit` partial-failure modes (the four non-degradable reason codes at `skills/claim-coverage-audit/SKILL.md §2`; emitted as `E-COVERAGE-AUDIT-FAILED`) do **not** halt the Ph2 cycle; the Planner proceeds to Phase 4 with `last_coverage_score` un-updated. Rationale: SK-NEW-B's output (the coverage signal) is *advisory* — it informs the Evaluator's Step 4 / Step 8 reads but is not the substrate the Generator drafts against. The eight `pre_phase_advance_check.py` clauses remain the binding Ph2 admission contract; the missing audit signal does not corrupt anything downstream, only widens the verification surface for user adjudication at cycle close.

The asymmetry is **not** a stage-specific exception or pragma; it is the binding rule for any future Step that fails partially. Future contributors evaluating row 4 for a new Step should ask: *is this Step's output the substrate downstream agents draft / read against (HALT), or is it advisory signal that informs but does not gate downstream reads (CONTINUE)?* The answer determines the row-4 polarity and the corresponding `phase_notifications.yaml` code class (E-* halt vs. E-* continue or W-* advisory).
...
| R-14 | Outcome (ii) BELOW_THRESHOLD audit on a long-tail uncovered claim list dispatches SK-NEW-C unboundedly, blowing the per-Ph2-cycle verifier budget (e.g., 30 uncovered × `max_iterations=2` × `per_seed_cap=5` = 300 verifier probes from a single Ph2 entry; v0.10.1) | Medium | Medium (token / quota cost; potential session abort; distinct from R-1 which is per-iteration cost on a single SK-NEW-A run, and from R-12 which is the SK-16 red-link queue surface) | Inline parallel-fanout cap of 8 SK-NEW-C invocations per Ph2 cycle, sourced from `max_parallel_extend_snowball` in `reviews/classification.md` (default 8 per the S4.5 R2 surfacing). Excess uncovered claims are ranked by Rule-7a-criticality (claim-kind priority: result > mechanism > comparison > existential > theoretical commitment; ties broken by claim-locus order); the first 8 dispatch immediately, the rest defer with non-blocking `W-COVERAGE-FANOUT-CAPPED`. Deferred claims surface as MAJOR Evaluator findings with `propose_extend_snowball` remediation. The cap is anchored at `agents/planner.md §Phase 3.8`; consumed `run-phase-2/SKILL.md §4 Step 0.5` references the contract rather than restating. |
