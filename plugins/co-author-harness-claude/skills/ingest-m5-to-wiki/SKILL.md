...
pattern_source: PROJECT_BOOTSTRAP.md §3 Step 5 + §4 Four Couplings table; knowledge/LLM wiki/CLAUDE.md §Operations/Ingest; AGENT_ORCHESTRATION.md §8.5 (Coupling C) — SK-17 is the M5-timed analogue
version: 1.0
---

# Ingest M5 to Wiki

You are materializing **Coupling D** — the final Research→Wiki hook in the four-coupling synergy program. This skill converts a project's submission-bound M5 manuscript into a complete (non-stub) source page in the peer `knowledge/LLM wiki/wiki/sources/` directory, with full bibliographic metadata, a structured claim summary, and inbound wikilinks from every concept the paper grounds.
...
2. **Wiki exists.** The path `knowledge/LLM wiki/wiki/` is reachable, with subfolders `sources/`, `concepts/`, `entities/`, `syntheses/`, and files `index.md` + `log.md`.
...
4. **Projected wiki-key is resolvable.** Either the project's CLAUDE.md carries a `projected_wiki_key` field, or the manuscript has final author/year/keyword information from which a key can be constructed per `knowledge/LLM wiki/CLAUDE.md §Source Keys` (`<first-author>-<year>-<keyword>`). If the projected key has a `-draft` suffix, finalize it by dropping the suffix — the M5 version is no longer a draft.
5. **No prior non-stub ingestion.** Check `knowledge/LLM wiki/wiki/sources/<key>.md`. If a page exists with `grounding_status: full`, stop and ask the user whether this is a revision (update in place) or a different paper (new key).
...
5. Read any existing `knowledge/LLM wiki/wiki/sources/<key>.md` (if the user is overwriting a stub or draft) so you preserve fields the stub got right and only replace what the full read changes.
...
Create `knowledge/LLM wiki/wiki/sources/<key>.md` with this frontmatter:
...
1. Append a row to `knowledge/LLM wiki/wiki/index.md` under `## Sources`, placed alphabetically by key. Mark as non-stub (no "(stub)" suffix).
2. Append a log entry to `knowledge/LLM wiki/wiki/log.md` under the current date with operation `ingest | M5 — <short project name>`. Record: the source key, the projected concept-grounding follow-ons (for SK-16), the wiki corpus size pre/post.
...
**This phase fires only on the *first* invocation of SK-17 in the workspace.** Check `DRIFT_LOG.md` or `knowledge/LLM wiki/wiki/log.md`: if no prior `skill | SK-17` or `ingest | M5` entry exists for SK-17, this is the first run. On first run:
...
If `knowledge/LLM wiki/wiki/sources/<key>.md` already exists:
