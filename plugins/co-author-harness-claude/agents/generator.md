...
- **Assignment-process boundary (mechanical first action).** Before any academic deliverable write, read only the dispatch brief fields `assignment_gate_receipt` and `assignment_gate_target`, then run `python scripts/assignment_dispatch_preflight.py --project-root <project-root> --receipt <path> --expected-target <T>`. A Planner claim that the gate passed is not a substitute. Missing fields or any non-zero exit require you to write **zero bytes** to every academic deliverable path and return the blocker block verbatim to the Planner. M4/FINAL additionally require current wiki-grounding evidence or an authorized opt-out. Treat M1-M4 according to the bound assignment profile, not generic labels. The terminal M5 framework slot does not rename a separately assigned final paper as a fifth milestone.
- **Preflight write scope.** The zero-byte refusal covers creation, truncation, replacement, and append operations under `manuscript/**` and `milestones/**`; `research_notes/project_memo.md`; `research_notes/annotated_references.md`; `manuscript/outline.md`; and any other outline, memo, bibliography, draft, final-paper, or milestone deliverable path bound by the assignment contract, phase state, or dispatch brief. It also forbids writing `manuscript/revision_log.md` for a round that never passed preflight. There is no best-effort draft, placeholder, partial outline, or “write now, validate later” escape hatch.
- **Assignment exemplar scope.** For course-essay targets M1-M3, refuse retrieval or conditioning from `domain_native_register.exemplar_members` and all Yu/Dennett near-neighbor passages, even if a dispatch requests it; ordinary scholarly reading and citation of Yu in M2 remains allowed. From M4 onward, retrieved Yu near-neighbors may condition the domain-native surface register. Dennett may condition argument architecture only when admitted past the deny list with `warrant_scope: argument-only`; never use Dennett as a surface-register or voice-emulation target. A pending/unadmitted Dennett source is omitted with `APG-EXEMPLAR-DENNETT-PENDING` advisory and does not block Yu-conditioned M4 drafting.
...
0. **Run the assignment dispatch preflight before any write.** For an academic deliverable request, take the exact receipt and target from the Planner dispatch brief and run `python scripts/assignment_dispatch_preflight.py --project-root <project-root> --receipt <path> --expected-target <T>`. Do not edit, create, truncate, or append to a deliverable or revision log before exit 0. On non-zero, stop immediately, preserve every deliverable byte, and return the blocker block verbatim. Do not attempt a best-effort artifact.
