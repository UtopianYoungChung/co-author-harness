...
## 9.5 Post-first-live-run refinements (observed 2026-07-14, epochs 2–3; for next iteration)

The first real ingestion (`--add-exemplar dennett-1987-intentional-stance`, applied as epoch 3 at `1d46ca4`) surfaced four implementation gaps. None blocked the run; all are recorded here so the next Codex round hardens them rather than rediscovering them:

1. **Gate 2 over-matches dry-run artifacts.** A dry-run's own ledger row + snapshot trip "uncommitted prior re-pin diff," forcing a commit between dry-run and apply in the same sitting. Intent was unlanded *profile* changes. Fix: gate 2 should ignore diffs consisting solely of `dry_run: true` ledger rows, derived md, and snapshots.
2. **Dry-runs consume epoch numbers.** Ledger shows epoch 2 = dry-run, epoch 3 = the apply; profile `pin_epoch` is an event counter, not an applied-state version. Monotonicity holds so bindings work, but either stop allocating epochs to dry-runs or document event-sequence semantics in the SKILL.md.
3. **Interactive confirm blocks headless callers.** The apply prompts `[yes/no]` on stdin — correct for humans, hangs automation (it hung two shell invocations tonight). Add `--yes` for pre-confirmed non-interactive use; keep the locked-role drop token separate.
4. **Test hermeticity vs live re-pin state.** `repin_register_smoketest.fixture()` and `domain_native_register_smoketest` loaded the LIVE profile, so the first real re-pin broke three suites (surface==argument equality, absolute `1.0.1`/epoch-2 asserts, duplicate-add refusals); the frozen F1 pass-fixture also went stale by design (the staleness gate working). Interim fixes shipped at `cfe071b`: scope-filtered baseline profiles in both suites + a fixture refresh (see `scratch/refresh_f1_fixture.py`, session-local). Proper fix: canned baseline profile fixture checked into `scripts/fixtures/`, and promote the F1 refresh script into `scripts/` as the standard post-re-pin maintenance step invoked (or at least named) by the skill.
