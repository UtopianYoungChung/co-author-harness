...
- **Reader-accessibility commitment (C-5, binding):** every revision must be accessible to a careful human reader at first read. The package-local reader-accessibility profile defines Check 8's A–H scope, thresholds, severity, transitions, and aggregation; `SAFEGUARD_LAYER.md` and the overlay implement that contract. Accessibility is **extraneous-load reduction**, not intrinsic-load collapse.
