...
- SK-NEW-D `inherit-snowball-from-wiki` (S6 — cross-project seed inheritance).
- Documentation amendments: `EXTERNAL_VERIFIERS.md §1.5` named-executor lines; `AGENT_ORCHESTRATION.md §8.6` Coupling E.1 retired/re-registered; final `references/SKILL_REGISTRY.md` SK-NEW-A/B/C/D consolidation entries (S5 — binding semantic-review).
- `plugin.json` version bump 0.9.0 → 0.10.0; `CHANGELOG.md (unreleased)` heading dated; `README.md` headline version updated (RC gate).
- Architecture- and strategy-doc amendments mirroring the four S4 binding decisions anchored at `agents/planner.md §Phase 3.8` (four-outcome handler / inline parallel cap of 8 / S4.5-delivered regression hook / halt-vs-continue asymmetry rationale); candidate for v0.10.0 RC documentation pass per S4 close-notes.
