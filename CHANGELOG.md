...
*Thresholds fixed (23 September 2026).* The review owner fixed the semantic acceptance thresholds before any run was scored: pooled recall ≥ 0.80 with every AC class detected on the held-out defects fixture, at most one false positive per run on the held-out controls, three runs per fixture judged on the median, and inter-rater disagreement reported rather than adjudicated. Recorded in `docs/evaluation/argument-coherence-baseline.md` §4.3. Semantic qualification remains held until an independent reviewer curates held-out fixtures and runs the scored evaluation against them.
