...
The `.plugin` and `.zip` snapshots are no longer committed. They were 8 MB of every install and had to be rebuilt after each commit. `.github/workflows/release.yml` builds the bundle for each `vX.Y.Z` tag. It refuses a tag that differs from `version.json`, runs source parity and loader-compat, and attaches `co-author-harness.plugin`, an identical `.zip` and `SHA256SUMS` to the tag's release; the notes come from `scripts/release_notes.py`. The newest bundle stays at `releases/latest/download/co-author-harness.plugin`. The README has an Install section for the marketplace (including auto-update, which Claude Code leaves off for third-party marketplaces), Desktop/Cowork upload, and Hermes.
