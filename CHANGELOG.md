...
**Chung academic-voice pass (2026-08-21).** New optional skill **SK-49 `chung-academic-voice-pass`** plus `references/chung_academic_voice_guidelines.md`. On-demand audit or explicit draft overlay for the registered authorial register (technical representation joined to institutional consequences). Not a STYLE_COMMITMENTS C-n and not auto-dispatched. Catalog, command-surface, and capability rows are `degraded` / prompt-mediated. DEST-PROTECTED, SK-32 CLOSED, no scholarly CLEAN.
