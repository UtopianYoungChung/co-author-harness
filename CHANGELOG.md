...
**The portable root `plugin.json` and the native-manifest rule are removed (26 September 2026).** With Hermes gone, nothing loaded the Agent Plugins v1.0.0 manifest. `plugin.json` and `scripts/agent_plugin_v1.py` are deleted; `version-check.py` no longer gates a portable manifest or refuses a native `plugin.yaml`; `update_version_manifests.py` advances `version.json` and the Codex host identity only; and the release tools (`build-plugin.py`, `build-release-zip.sh`, `release-gate.sh`, `release_manifest_check.py`) and `manifest-coherence-check.py` read `.claude-plugin/plugin.json` as the host manifest. How to apply: `version.json` stays the only identity authority; the host packs are `.claude-plugin/` and `.codex-plugin/`.

**Hermes support is removed (26 September 2026).** Hermes is no longer used. `scripts/install_hermes.py`, `scripts/hermes_plugin.py`, `scripts/piw_hermes_host.py`, `scripts/hermes_host_smoketest.py` and `references/HERMES_DESKTOP.md` are deleted; the `hermes-hooks-jsonl` adapter leaves `scripts/piw_native_host.py`, and its contract-kernel component, two destination-registry writers and fixture-registry suite go with it. Hermes is also removed from the docs that named it. How to apply: the registered trace adapters are now `codex-jsonl` and `claude-code-jsonl`. Dated plans, work orders and release history still mention Hermes as a record of what was built.

**The shipment-v2 profile's kernel pin is checked, and the unscoped `adhoc_review` dispatch path has a regression suite (26 September 2026).** `compatibility_profile.json` publishes the contract-kernel digest to consumers, but no check compared it with the kernel: the kit aggregate excludes the profile, and `schema_runtime_check.py` compared only the projection's copy. The pin was stale from at least `d3fcd71` through `50d42a7` (including the v0.51.0 release commit) and again at `d220129`, because the re-pin procedure used for those commits refreshed the kernel and projection but not the profile; `71908a4` and `064b397` corrected it by hand. `schema_runtime_check.py` now refuses a profile whose `contract_kernel.sha256` differs from the kernel ("compatibility profile contract-kernel pin drift"), and `contract_kernel_coherence_smoketest` gains N4, which leaves only that pin stale in a rebound hermetic tree. Applied to the `d220129` and `50d42a7` trees, the check blocks both. `full_run_enforcement_surfaces_smoketest` gains 12 checks for `008013f`, which lets a brief declaring exactly one `run_scope: adhoc_review` dispatch read/review work with `FRC_PARENT_SCOPE` unset: such a brief dispatches from a native project and from a governed research tree, and prohibition wording ("do not write or edit") is not read as a mutation; a review-then-rewrite, an imperative revise, a generator drafting request or `run-generator-session` is refused (`FRC-PROSE-FORBIDDEN`); conflicting declarations, a malformed parent scope and `FRC_REQUIRE_SCOPE=1` do not unlock it; an undeclared manuscript brief is refused with the declaration hint; and the exemption grants no manuscript write. Eight of the twelve fail on the gate before `008013f`. The dispatch screen is heuristic: indirect requests such as "You should rewrite…" pass it, and the child's argument-bearing writes are then refused by the write gate, as a live Claude Code session confirmed.

**The Codex role setup refuses the package and governed workspaces, and the two CI regressions from the Codex adapters are fixed (26 September 2026).** `71908a4` added the Codex `PreToolUse`/`Stop` adapters and `scripts/codex_agent_setup.py`, and turned all three CI jobs red. First, three `subprocess.run` calls in `loader_compat_portability_smoketest.py` used text mode without an explicit `encoding`/`errors`, which `subprocess_text_policy_smoketest` refuses on both runners (CI named two; the third surfaced once they were fixed). They now pass `encoding='utf-8', errors='replace'`. Second, the setup script writes files but had no destination-coverage entry. Like `init_governed_workspace.py`, it is now a user-invoked setup tool (class `excluded`, sha256-pinned), and it gained the same boundary: `--agent-dir` and `--config-output` are refused, in dry runs too and before anything is written, when they lie inside the harness package (`DEST-MISROUTED`), are a directory enclosing it (`SETUP-ENCLOSES-PACKAGE`), or lie inside a governed workspace (`SETUP-IN-GOVERNED-WORKSPACE`). Roles for governed research therefore belong in the personal `~/.codex/agents`; project-level `.codex/agents` remains available for projects outside every governed workspace. `case_codex_native_roles` covers all three refusals and fails on the unguarded script. `AGENTS.md` now names two setup-time exceptions to the producer boundary, `docs/agent-instructions/codex-host.md` documents the refusals, and the release notes' Codex line points to the Codex host setup instead of saying "skills only".

**The README and release notes say how to install the harness in Codex (26 September 2026).** The commands are `codex plugin marketplace add UtopianYoungChung/co-author-harness`, then `codex plugin add co-author-harness@joseph-chung-co-author-harness`. Codex loads the 44 skills (the Codex hook adapters and role setup followed in `71908a4`; see above). Its install carries the Codex manifest's release version as a label, so the README also shows how to refresh to the latest `main` (`codex plugin marketplace upgrade`, then the same `codex plugin add`) and how to read the installed commit. All of these were exercised with Codex CLI 0.157.1 against this repository's marketplace, where the install matched `main` byte for byte and `codex debug prompt-input` listed all 44 skills.

**Linux fixture supervision can be exercised on hosts without a user manager (26 September 2026).** `scripts/analysis/systemd-user-sandbox.sh <command>` runs one command under a private systemd user manager. It enters its own mount namespace, mounts cgroup v2 and tmpfs copies of `/run/systemd` (holding the `/run/systemd/system` marker systemd requires) and `/run/user`, starts `systemd --user` inside `/user.slice/user-<uid>.slice/user@<uid>.service` as a login session would, runs the command, and exits with its status. The host's mounts are unchanged afterwards. It needs root; it refuses without root or arguments, and on a host whose user manager already works it runs the command directly. This is the setup that reproduced and verified the Linux timeout fix in v0.51.0. Through it the fixture preflight passes all 223 checks in this container, and a registry suite runs under full supervision. The supervisor's cgroup-v2 refusal now names the tool.

**The same draft-governance contract can be published in more than one lane (25 September 2026).** This fixes the v0.51.0 known defect in which two byte-identical publication intents collided on Linux. `draft_governance_publish.prepare_contract` named its transaction after the contract's digest alone, and the contract records no lane. Publishing an unchanged contract under a second evidence label (here `producer-smoke`, then `typed-producer-smoke`) therefore reused the first lane's transaction, whose recorded intent names the first lane's paths, and the second publication was refused with "existing transaction intent differs; recovery required". The same happens to any rerun that prepares an unchanged artifact under a new label. On the hosted Windows runner the two contracts happened to differ, so the suite passed there; why they differ was not established, and the fix does not depend on it. The transaction id now hashes the contract's lane-relative path together with the contract. A lane already committed under the contract-only id keeps that id when its marker matches exactly, so re-preparing an unchanged contract stays idempotent for existing evidence. `production_evidence_publisher_smoketest` gains a case covering: a rerun in the same lane keeps its transaction and marker; the same contract in a second lane gets its own transaction and validates; a lane committed under the old id re-prepares under that id. The case fails on the previous publisher with the production error, and the suite passes on Linux (191 s).
...
The `.plugin` and `.zip` snapshots are no longer committed. They were 8 MB of every install and had to be rebuilt after each commit. `.github/workflows/release.yml` publishes the bundle whenever a push to `main` changes `version.json`: it creates the `vX.Y.Z` tag at that commit, so the version bump is the whole release step. A pushed tag that matches `version.json` or a manual run works too, and an existing release is not rebuilt unless its own tag is pushed. The workflow runs source parity and loader-compat, then attaches `co-author-harness.plugin`, an identical `.zip` and `SHA256SUMS`, with notes from `scripts/release_notes.py`. The newest bundle stays at `releases/latest/download/co-author-harness.plugin`. The README has an Install section for the marketplace (including auto-update, which Claude Code leaves off for third-party marketplaces), Desktop/Cowork upload, and Hermes.

**A fixture-qualified extractor is re-qualified wherever its receipt is validated (25 September 2026).** Two gaps surfaced once the registry ran with a fixture-qualified `pdftotext`. First, the receipt's `version` and `sha256` were self-reported, so a receipt edited to claim version 99.0.0, or a fabricated hash, still validated; `product_assurance_smoketest`'s intended-red extractor cases caught it. `validate_extractor_identity` now re-establishes a qualified identity on the validating host. The executable must be on `PATH` with exactly the declared name, size and hash, report the declared version, and still reproduce the fixture. Otherwise the receipt is refused (`EXTRACTOR-UNSUPPORTED` or `EXTRACTOR-IDENTITY-MISMATCH`). The reference identity keeps its constant check. Second, `build_activation_fixture` pre-seeded the committed reference raw extract and page map over its output paths. On a fixture-qualified host a rebuild therefore found outputs diverging from the committed publication marker (`scholarly_lifecycle_integration_smoketest`). It now pre-seeds the active extractor's own raw bytes and rebased map.

*Known open defects surfaced by the first CI registry runs.* The Windows job (`fb09fab`, before the fixes above) passed 78 of 114 cases. About 20 failures share one cause. The hosted runner's `TEMP` is an 8.3 alias (`C:\Users\RUNNER~1\…`), and resolvers such as `reader_accessibility_policy.resolve_domain_native_register` (`relative_to(wiki_root)`) compare resolved long paths with unresolved short ones and raise `ValueError`; the same mix is the likely cause of the `APG-RECEIPT-INVALID` receipt-path and `admitted root handle identity mismatch` failures. Any Windows user whose profile path is 8.3-shortened is exposed. CI now points `TEMP`/`TMP` at the runner's long-path temp directory, as on the author's host, but the resolvers still need to canonicalise both sides. `release_qualification_controller_smoketest` fails on the hosted Windows runner because the release-gate facade's detached worker is refused ("detached Windows worker cannot escape the enclosing Job"). It passes on the maintainer's Windows host under the same runner, and the cause of the difference is not yet established. Suites that commit in scratch repositories needed a git identity, which CI now configures. With these changes the Windows job at `6a5ea15` passed 110 of 114 cases. `semantic_qualification_consumer_smoketest` is UNAVAILABLE (sibling wiki absent). `migrate_lab_iteration_derived_handoff_smoketest` passes 8 of 23 checks; the rest re-derive policy from the author's `knowledge/LLM wiki`. The release controller is still refused. The fourth failure is fixed below.

**Two cases declare that they need the maintainer's workspace or host (25 September 2026).** `migrate_lab_iteration_derived_handoff_smoketest` now declares `requires_workspace_paths: knowledge/LLM wiki/graphify-out/graph.json`, the pinned corpus from which it re-derives the reader-accessibility policy. The registry gains a second precondition, `unavailable_on_github_hosted`: a list of `os.name` values for which the case is `UNAVAILABLE` when `RUNNER_ENVIRONMENT` is `github-hosted`. `release_qualification_controller_smoketest::fixture-owner` declares `nt`. Both are treated like the existing workspace precondition: a failure by default, tolerated only under `--allow-unavailable`, never counted as a pass, and never written as canonical evidence. The maintainer's workspace and self-hosted runners still run both cases. Hosted CI no longer runs the 8 of 23 migration checks that pass without the wiki.

**`pre_phase_advance_check` writes UTF-8 on every console (25 September 2026).** Its success line ("section … → Ph4") and its findings carry non-ASCII text. A piped stdout on Windows defaults to the ANSI code page, so a passing gate run crashed with `UnicodeEncodeError` after its verdict. That is how Claude Code on Windows captures script output, and how `pre_phase_advance_phase_state_smoketest` calls it. `main()` now reconfigures stdout and stderr to UTF-8, as the package's other CLI entry points do. The failure reproduces on Linux under `PYTHONIOENCODING=cp1252` and is gone with the fix.

*Known open defect: two byte-identical publication intents collide on Linux.* `production_evidence_publisher_smoketest` publishes the same contract in its `producer-smoke` and `typed-producer-smoke` lanes. On Linux both derive transaction digest `0c67bda3b533`, and the second `prepare_contract` is refused with "existing transaction intent differs; recovery required". The suite passes on the hosted Windows runner (`6a5ea15`), and why the platforms differ is not yet established.

**Linux suite timeouts keep their diagnostic and output (25 September 2026).** Under systemd supervision a timed-out suite yielded `FIXTURE-PROCESS-LIVE` ("systemd anchor result is absent") with empty stdout and stderr, where Windows yields `FIXTURE-PROCESS-TIMEOUT` with the captured bytes. That failed 7 checks in CI's Ubuntu `fixture_infrastructure_check`. Three things fired at `execution_deadline` (the deadline minus the cleanup reserve): the anchor detected the timeout and began its teardown, the parent's wait loop declared the unit over budget and killed it, and the unit's `RuntimeMaxSec` hard stop expired one startup latency later. The anchor never lived long enough to report. Every timed-out suite was affected, whatever its budget. In `fixture_process_supervisor.py` the parent now computes the proven hard stop (activation plus `RuntimeMaxSec`) and sends the anchor and worker an execution deadline of `min(deadline - reserve, hard stop - reserve/2)`. The parent then waits for the anchor's report, or for systemd to end the unit, and kills only a unit that outlives the hard stop. `RuntimeMaxSec`, the startup proof and the watchdog at the original deadline are unchanged. The preflight's infrastructure-error probe now runs with a 3-second budget, not 0.25 seconds. Supervised startup takes about 0.3 seconds on Linux, so under the old budget the suite never wrote the bytes the probe checks for.

Verified on this repository's Linux container under a real systemd 255 user manager: cgroup v2 and a `/run/systemd/system` marker in a private mount namespace, with the manager at `/user.slice/user-0.slice/user@0.service` as a login session places it. At `f0fc38c` the preflight reproduced CI's 7 failures exactly. With the fix all 223 checks pass, and timeouts of 0.8, 1.5 and 3 seconds return `FIXTURE-PROCESS-TIMEOUT` with the suite's exact stdout and stderr. The stalled-anchor, stopped-client and owner-loss checks stay within their bounds (0.48 s against 1.5 s, 0.74 s against 1.2 s).

**Installers can create a governed workspace, and read-only modes need none (25 September 2026).** Outside the author's workspace every governed writer refused with `DEST-UNGOVERNED`: project bootstrap, the documented `/quick-deterministic` command, and even modes that write nothing (`check8_g_prefilter.py --stdout-only`, `render_lifecycle_state.py --check`, and `run_all.py --project-root … --stdout` with both project reports skipped). No document said how to create the routing manifest the guard looks for.

*What.* New `scripts/init_governed_workspace.py <dir>` creates `governance/output-routing/output_routing.yaml` (exclusive create, never overwritten) and the `outputs/co-author-harness/staging/` lane. It refuses the package tree, a directory that encloses the package, and a directory already inside a governed workspace. The three read-only modes call the write guard only when they will write. The fail-closed rule itself is unchanged; `AGENTS.md` names the initializer as the one setup-time exception to the producer boundary. `README.md`, `PROJECT_BOOTSTRAP.md` and the quick-deterministic skill document the path.

*How to apply.* Verified end to end on a host with no workspace: initialize, bootstrap into `<ws>/outputs/co-author-harness/staging/w1/r1`, then run the documented pre-flight with `--project-root` and `--out`. `destination_capability_smoketest` gains 14 checks for the read-only modes (run against a protected project, and against an ungoverned host for `run_all`) and for the initializer's create, idempotent, nested and in-package cases. Each read-only check fails on the previous script.

**Hosted CI runs the fixture registry in a sandbox workspace (25 September 2026).** The `fixtures` jobs check the package out as `ws/platform/co-author-harness` beside a sandbox routing manifest, mirroring the author's layout, and install a Poppler `pdftotext`: `poppler-utils` on Ubuntu, and the poppler-windows 24.02.0 release pinned by archive SHA-256 on Windows. A registry case can now declare `requires_workspace_paths`. `semantic_qualification_consumer_smoketest` declares the sibling knowledge wiki's qualification script. A missing precondition makes the case `UNAVAILABLE`, which is a failure unless the caller passes `--allow-unavailable`. Even then it is listed and never counted as a pass, and the flag is refused when the run would write canonical evidence. CI passes it; local runs and the release gate do not.

**Any pdftotext that reproduces the conformance fixture is a qualified extractor (25 September 2026).** `source_extract.py` admitted exactly one executable: `pdftotext.exe` 24.04.0, 343,552 bytes, sha256 `640b9a93…`. On macOS, on Linux, and on Windows without that exact build, source extraction, scholarly evaluation, the product gate and milestone or terminal close could never succeed, and 24 registry suites failed on the pin alone. The C0 closure spec already stated the admission rule: a different executable is "unsupported until the same fixture qualifies it". Reproduced before the change: Ubuntu's `poppler-utils` 24.02 differs from the reference raw output only in line endings, and its `utf8-lf-v1` normalization of `miniature.pdf` equals the committed `expected_utf8_lf_v1.txt` byte for byte.

*What.* `resolve_pdf_extractor()` still prefers the reference build wherever it sits on `PATH`, so an earlier shadow cannot displace it. Otherwise it admits the first `pdftotext` or `pdftotext.exe` that reports a version and reproduces the fixture. The fixture PDF's own hash is checked first. The receipt now records the executable actually used, where it previously always copied the reference identity, and binds that executable's own hash as a publication precondition. A fixture-qualified identity also carries `qualification: {method: fixture-conformance-v1, fixture_id, fixture_sha256, normalized_sha256}`. `c2_evidence_validation.validate_extractor_identity` accepts the reference identity unchanged, or a qualified one whose qualification names the committed fixture exactly. The receipt schema gains the optional `qualification` object. Test support rebases the committed page-map seed's `raw_sha256` onto the active extractor; pages and named spans are normalized offsets and do not move.

*Why.* The pin tied evidence integrity to one machine's binary instead of to the behaviour the evidence depends on. Qualification by the committed fixture keeps the guarantee that matters: the canonical text is the text the fixture expects.

*How to apply.* `source_extract_smoketest` passes on Linux with `poppler-utils` 24.02. It gains controls: a runnable impostor that reports a version but forges text is refused `EXTRACTOR-IDENTITY-MISMATCH`, and a forged, re-pointed or stripped qualification fails validation. Disabling the conformance check fails the suite. The trade-off is explicit: a `PATH` binary that reproduces this one small fixture is admitted. `references/semantics_manifest.v1.json` re-pins the schema, the validator and the adapter.

**CI's structural checks and fixture preflight no longer depend on the author's workspace (25 September 2026).** `structural-checks` had not passed on `main` since 2026-07-19. At `80c5370` the `checks` job stopped at `destination-coverage-check.py`: four vendored `scripts/citation_gate` writers had never been registered (since `1241a76`), and `scholarly_assurance_fixture_support.py` had changed without a re-pin. Both `fixtures` jobs stopped at `fixture_infrastructure_check.py`, whose two failure-transcript probes captured into bare temp directories. Those are writable only on a host that already has a governed workspace, so every hosted runner refused them with `DEST-UNGOVERNED` and the authoritative registry step never ran.

*What.* The citation-gate caches (`~/.claude/cache/crossref`, `LOCATOR_CACHE` or `~/.claude/cache/locator-text`) are registered as `excluded` with their exact destinations, their two vendored tests as `test_only`, and the support file is re-pinned after review (it adds a key before an existing write, no new write site). The transcript probes now build the smallest governed workspace, a routing manifest in a temp directory, and capture inside its staging lane, which the guard allows on every host. Three suites that failed on POSIX for their own reasons are fixed. `destination_capability_smoketest` builds its directory alias with `os.symlink` off Windows instead of calling `cmd /c mklink`, so the alias check now runs on POSIX. `source_extract_smoketest`'s `os.replace` stubs accept the `dir_fd` keywords the POSIX path passes; Windows keeps the transient-retry assertions, and POSIX asserts a single fail-closed attempt with no debris. `release_qualification_controller_smoketest` no longer demands a creation token from the worker that the `crash_after_supervisor_spawn_before_journal` fault deliberately kills; a live worker must still yield one.

*Why.* A drift alarm that cannot pass anywhere but one machine is not an alarm. Every "114/114" in the log since July rests on runs no one else could reproduce.

*How to apply.* This does not make the fixture registry green on a hosted runner. Suites that need the pinned `pdftotext.exe`, a governed root, or the sibling `knowledge/LLM wiki` checkout still fail there. The predicate-census step still needs `docs/analysis/generated/fixture_manifest.json` regenerated from a full green run.

**The Claude Code hook guards harness territory, not folder names (25 September 2026).** Plugin hooks run in every session once the plugin is enabled. With `FRC_PARENT_SCOPE` unset, which is every installer's default, the gate denied any Write or Edit whose path had a segment named `research`, `manuscript`, `milestones` or `60_workbench` anywhere on disk, and any subagent brief that mentioned a manuscript. Reproduced before the change: `~/Research/thesis/notes.md`, an unrelated web app's `src/milestones/api.py`, and the package's own example trigger "Evaluate the manuscript" were all refused. Declaring the read-only `adhoc_review` scope did the opposite: it checked only `manuscript/`, so an Edit to the canonical `milestones/M4_complete_paper_draft.md` was allowed.

*What.* Without a scope, argument-bearing paths are denied only inside harness territory: a native project (an ancestor holding `reviews/phase_state.json` or `reviews/assignment_contract.json`) or a governed workspace root discovered by `destination_capability`. Briefs naming `run-generator-session` stay refused everywhere; briefs naming a manuscript are refused when the session's cwd is in harness territory. Under a declared scope, every lifecycle artefact (`manuscript/`, `milestones/`, `submission_bundle/`) routes through `full_run_contract_check.py authorize`, so `adhoc_review` refuses the M4 draft, and argument-bearing paths inside a governed root but outside the staging and private-shipment lanes are `DEST-PROTECTED` under every scope. Terminal markers match as whole words ("fig.4" is not "G.4"). A host re-invoked Stop (`stop_hook_active`) is not re-blocked on structured state alone, which the model cannot repair inside the turn; a fresh terminal claim still blocks. A payload that is not a JSON object now takes the unreadable-payload path, so it is denied under an active scope instead of crashing to a non-blocking exit 1.

*Why.* A folder name is not evidence of a lifecycle. The deny rule protected nothing on an installer's disk and blocked ordinary work there, while the scoped rule left the one artefact the ladder governs unprotected.

*How to apply.* `scripts/full_run_enforcement_surfaces_smoketest.py` gains 17 checks (72 total); the new cases fail on the previous hook. Ordinary `/run-draft` and `/run-iterate` need no scope. To drive a governed run from Claude Code, set `FRC_PARENT_SCOPE` before the host starts, in the launching shell or the `env` block of `.claude/settings.json`; `references/CLAUDE_CODE_HOST.md` now says so.

**The shared Reflector snippet is back inside its own budget (21 September 2026).** `references/_snippets/reflection-grounding.md` had grown to 1,578 tokens against the `< 1500` ceiling `reflector_split_parity_smoketest` pins, so that suite had been failing on a budget the file itself set. 1,578 -> 1,463, with 37 tokens of headroom.

*What.* Only duplication and verbosity were cut; no rule, path, finding code, phase name or version token was removed, checked mechanically against the previous bytes rather than by eye. Three genuine repetitions went: the `Grounding enforcement` invariant restated the Binding constraint word for word and is now folded into it; the three-filter gatekeeper was stated in the writes list and again as an invariant, so the list points at the invariant; and `phase_state.json`'s never-mutate rule appeared in both `Writes (never)` and the milestone audit boundary. The milestone boundary also gains the paragraph break it was missing.

*Why.* The budget is not decoration. Both reflector splits runtime-bind this file, so every token here is paid on every probe and every close-out. Raising the ceiling to match the file would have made the test describe the file instead of constraining it.

*How to apply.* When this file needs new content, spend the 37-token headroom deliberately and re-measure with `python scripts/reflector_split_parity_smoketest.py`, which prints `snippet=<n>`. A budgeted shared file accretes; expect this to recur, and cut duplication before cutting doctrine.

**Argument coherence is a reviewed obligation, not a hope (21 September 2026).** Prose that is fluent, grammatical, on-topic and well-cited can still do no argumentative work, and every mechanical control in this package was blind to it by construction: `DETERMINISTIC_CHECKS.md` §9a keys on named-author application phrases and normative jumps, §9b–§9e on sentence length, definition topology, cognitive load and register. Reproduced before building: on a synthetic Method section carrying an unused source-status aside between a premise and its `therefore`, an unpaid research promise, and an unmarked re-specification of a defined term, the full deterministic set returned four passive-voice findings and zero coherence findings, and §9a produced no candidates at all. The same result held on a seeded five-defect fixture.

*What.* One shared obligation in `references/ARGUMENT_COHERENCE.md`: determine the paragraph's purpose from the text, assign each substantive sentence a contribution, read the transitions, and check the document's research commitments. Five defect classes AC-1…AC-5, four required parts per finding, and an explicit positive-control list — implicit transitions, legitimate background, qualifications, counterarguments, relations established earlier in the section, and authorial voice under C-7 — so the obligation cannot be turned into a signposting rule. Executed by new `SAFEGUARD_LAYER.md` Check 9, fed by the new `DETERMINISTIC_CHECKS.md` §9f pre-filter (`scripts/coherence_prefilter.py`), whose primary product is a coverage denominator rather than a candidate queue. Enforced on the `project_independent` route as a non-suppressible `argument_coherence` required check validated by `scripts/coherence_review.py`, and on the governed route through `scholarly_evaluation.PROFILE_MINIMUM`.

*Why.* A conversational manuscript edit passed a style-lint probe clean, and no independent semantic evaluator ran on those bytes at all. That is a routing defect, not a weak judgment: nothing was judged. The two closest existing skills could not have caught it either — `/sentence-level-pass` judges how a sentence is built, `/narrative-structure-pass` judges the document arc at section scale — and both now say so rather than letting a clean craft or arc result read as a coherence result.

*How to apply.* Required coverage is every changed prose unit **and its immediate neighbours**, measured against the author's original bytes, because the defect that hides from ordinary scope discipline is the sound edit that disconnects an untouched neighbour. A no-change run still owes a full-scope review. Inspection may extend past the write scope; findings there confer no write authority. Mechanical validation establishes evidence integrity and coverage and says so in its own output — `semantic_correctness_established: false` on every passing run. Semantic qualification is **held**: acceptance thresholds are unset and must be fixed prospectively with the review owner, and every fixture here carries `independently_curated: false` because it was authored in the same session as the obligation it tests. Conversational manuscript application remains an unsupported route with no coherence enforcement, and `docs/evaluation/argument-coherence-baseline.md` names it as such rather than claiming a gate the package does not have.

*Thresholds fixed (23 September 2026).* The review owner fixed the semantic acceptance thresholds before any run was scored: pooled recall ≥ 0.80 with every AC class detected in at least two of three runs on the held-out defects fixture, at most one false positive per run on the held-out controls, three runs per fixture judged on the median, and inter-rater disagreement reported rather than adjudicated. Recorded in `docs/evaluation/argument-coherence-baseline.md` §4.3. Semantic qualification remains held until an independent reviewer curates held-out fixtures and runs the scored evaluation against them.

*Independent source review addressed (23 September 2026).* An independent, non-Anthropic source review of `2718107` returned six MAJOR findings, each an acceptance gap in the enforcement rather than a judgment error. Each was reproduced from invented text as a failing control in `scripts/argument_coherence_smoketest.py` before the production code changed; 14 of the 20 new controls failed on `2718107` and all 55 pass now.

- **Governed pass without a review.** `scholarly_evaluation.py` now validates the evaluation's top-level `coherence_review` against the artifact bytes, over the whole artifact, whenever the profile carries `argument_coherence`; absent, partial, stale or wrong-scope review evidence is a blocker, so the verdict cannot be `qualified`. The schema gains the `coherence_review` property, and the coherence modules and both coherence authorities join the verifier's watched dependencies.
- **Deletion masked by a distant edit.** `coherence_prefilter.changed_unit_ids` aligns before and after unit hashes in order (`difflib`) instead of testing set membership. A deleted or moved paragraph marks the surviving units on either side of the gap as affected, and a paragraph inserted a second time is a change.
- **Section scope truncated the read context.** `piw_coordinator.coherence_scope` now inventories the whole target and returns the write-scope unit ids separately. The requested section bounds what may be written, not what must be read, so a neighbour across a section boundary is required. `candidate_sha256` is now the hash of the whole target text.
- **One record for several sentences.** Each sentence record must hold one sentence of the inventory's segmentation. A record may span a boundary the splitter found only by declaring it an abbreviation, with a reason, and only a short form qualifies (`COHERENCE-SENTENCE-MERGED`). The splitter protects titles, common scholarly abbreviations and single initials, and keeps closing quotes with the sentence they close, which also fixes an exact-partition failure on sentences ending in a quotation.
- **Coherence rules unbound.** `piw_session.rule_bindings` binds `ARGUMENT_COHERENCE.md` and `SAFEGUARD_LAYER.md` in every contract, so a change to either invalidates an earlier completion, as a change to any bound rule does.
- **Commitments skipped or falsely located.** `commitment_occurrences` is required. Every required unit that states a promise, a definition or a question (new `definition` and `question` markers) is assessed with a row that names its unit, quotes it there, and states the assessment; a fresh draft is included. Each occurrence locator is `{unit_id, quote}` and must quote text that occurs in that unit.

The review's five full-registry failures were not addressed here: four assume a checkout parent or sibling layout that an isolated clone lacks, and one (`staging_authority_mode_smoketest`) removes `COAUTHOR_EXTRA_GOVERNED_ROOTS` instead of restoring it. None of those suites or their subjects was touched by this work order. Semantic qualification remains held; it runs on the fixed commit.

*Independent re-review addressed (24 September 2026).* The re-review of `80c5370` confirmed that all six original reproductions are now refused, and found three more acceptance gaps with new invented probes. Each was reproduced as a failing control before the fix (7 of 10 new controls failed; 65 pass now):

- **Deletion inside a run of identical paragraphs.** The alignment put the gap at one end of the run, so a unit claiming a caveat "appears three times" was not required when one copy went. Every position the gap could occupy is now affected.
- **Silent sentence merges.** The splitter protected a single capital and "et al.", so "option A. Costs remain high." was one sentence and one record could cover both. It now protects only forms that practically never end a sentence; every other boundary must be declared to be merged.
- **Commitment rows per unit, and self-carried commitments.** Every promise, definition or question sentence in the required scope needs its own row, whose quote lies inside that sentence; `carried` requires an occurrence outside the commitment's own unit.

**Citation coverage is bound to the citation, its source and its locator (20 September 2026).** Reviewer findings R2, R3 and R4 of the 2026-09-20 re-pass, which are one defect in three forms. Coverage was "any check's `claim_text` appears anywhere in the sentence", which established nothing: a check for the first clause cleared "and all autonomous systems are always safe" in the same sentence; a check naming Yu 2024 cleared a sentence citing Other 2025; and the inventory admitted only parenthetical author-year and `n.d.` forms, so an unsupported numeric claim was never inventoried at all. All three were reproduced here first, then re-checked against the reviewer's own saved fixtures.

The unit is now the **citation occurrence**. Each one asserts that the material in its scope is supported by a particular work at a particular locator, so `citation_targets()` resolves it to the source keys it names -- by author family and year, by `bib_numbers` for numeric labels, by key for `\cite{}` -- and to the page span it states. Only checks naming that work, at a page inside that span, count as coverage. `uncovered_residue()` then marks what those checks actually account for and reports what is left, so a clause nobody checked surfaces by name. Anything unresolved -- an unknown work, a numeric label with no `bib_numbers` mapping -- is itself UNCOVERED, which the auditor blocks on as `CIT-LOC-014`.

Residues below three content words are treated as framing ("Analysis shows") rather than claims. An earlier cut split the scope on coordination instead, which tore "a role and the agent playing it" in half and would have failed every real sentence; marking coverage and inspecting the remainder needs no guess about clause boundaries.

`scripts/citation_gate/` re-vendored, and `verify_locators_test.py` added there: hermetic unit cases for citation resolution and residue, needing no PDF, so the logic is testable wherever the tool ships. `test_citations` 19/19, with a new parity case asserting that the auditor's `SOURCE_CITATION_RE` and the verifier's `CITATION_RE` admit exactly the same citation forms -- a form only one of them sees is a citation that can escape coverage. `test_audit`, `draft_governance_smoketest`, `product_assurance_smoketest` unchanged; `contract-kernel-check` PASS at the unchanged `35d785ed5c45`.

**The auditor relays every conclusion, and must agree with the tool that reached it (20 September 2026).** Reviewer finding R1 of the 2026-09-20 re-pass. The line relay handled `FAIL`, `UNSUPPORTED`, `DISCLOSE` and `REVIEW` but not `UNCOVERED`, and never consulted a tool's exit status. A manuscript with one checked sentence and a second cited sentence with no check therefore had the verifier exit 1 reporting `UNCOVERED 1` while the auditor returned **no finding at all**: the coverage inventory added a fortnight earlier was printed by one process and read by none. Reproduced here against the reviewer's own `uncovered-author-year` fixture before repair.

`UNCOVERED` is now relayed as `CIT-LOC-014` at `inviolable`. A cited sentence no check covers is a citation with no evidence, which is what `CIT-LOC-000` already says about a whole manuscript, so it blocks for the same reason.

Each tool's exit status is now reconciled with what the auditor parsed from its output. A summary line proves the tool got far enough to have an opinion; it does not prove the opinion was understood. A non-zero exit that yields nothing the relay recognises, or a clean exit alongside a blocking result, raises an inviolable `CIT-LOC-002`: the two disagree, so the run clears nothing. Advisory-only output on a clean exit, and validation's REVIEW-only exit 2, remain legitimate and are asserted as such.

Tests: `test_citations` 18/18, two added covering the UNCOVERED relay and five agreement cases across both tools. Verified against the reviewer's saved fixtures: `uncovered-author-year` now yields one inviolable finding, and `positive-control` is unaffected. Their `partial-element`, `wrong-reference` and `uncovered-numeric` fixtures remain unblocked — those are findings R2-R4, one defect in three forms, not addressed here: sentence coverage is still a substring test, is not matched to the citation's own source and locator, and omits numeric and LaTeX citation syntax that the auditor itself recognises.

**An unexecuted citation gate clears nothing (20 September 2026).** On the cross-family reviewer's recommendation of 2026-09-20. `CITATION_GATE_TOOLS` used to win on precedence alone: an override naming a directory without the tools left the gate unexecuted and produced advisory `CIT-LOC-001` with **zero inviolable findings**, so one environment variable disabled citation checking entirely. After the previous commit shipped the tools, that was the cheapest remaining route past the gate, and the only one an operator could take deliberately.

`_resolve_gate_tools()` now ranks **completeness**, not specificity: the first location holding every tool wins, searching `CITATION_GATE_TOOLS`, then the author's working copy, then the bundled copy. A valid complete override still takes precedence and is silent. An incomplete one falls back to a complete gate and reports the misconfiguration as `CIT-LOC-001`, naming what was missing and where it fell back to, so the fallback is visible rather than silent.

The diagnosis and the consequence are now separate findings. `CIT-LOC-001` stays `default`: an environment fault is a statement about the machine, and an author cannot fix a missing dependency by citing better. It no longer stands alone. Whenever a gate does not execute - no complete gate anywhere, a missing PyMuPDF reported as `GATE_UNAVAILABLE`, an `OSError`, a timeout - an `inviolable` finding is raised beside it: `CIT-LOC-002` when nothing could be run at all, and the new `CIT-LOC-003` when a tool that was found could not complete. Absence of a verdict is not a clean verdict.

Tests: `test_citations` 16/16. Three existing tests encoded the old policy and were changed deliberately rather than worked around: tools-absent no longer means advisory-only; precedence is asserted for a complete override and for the fallback from an incomplete one; and `GATE_UNAVAILABLE` now asserts the advisory diagnosis together with the blocking consequence. `test_audit`, `draft_governance_smoketest` and `product_assurance_smoketest` unchanged; `contract-kernel-check` PASS at the unchanged `35d785ed5c45`.

Not yet shipped: until this is built into the snapshots and the installed caches are refreshed, the installed copies still carry the override bypass.

**The citation gate ships with the harness (20 September 2026).** The archive contained none of the three quote-binding tools: they lived only in the author's home directory. An installation without them emitted an advisory `CIT-LOC-001` and **zero inviolable findings**, so every claim that the harness mechanically blocks missing citation evidence held on one machine and nowhere else. `scripts/citation_gate/` now vendors `verify_locators.py`, `validate_sources.py`, `reconcile_citations.py` and their test, with a `PROVENANCE.json` recording the upstream path, the date and each file's sha256; a test compares the live bytes against that record so the two copies cannot drift silently. `_gate_tools_dir()` resolves `CITATION_GATE_TOOLS`, then the author's working copy, then the bundled copy, so an existing working copy still wins.

*A gate that cannot run is advisory; a gate that crashed still blocks.* Shipping the tools puts them on machines without PyMuPDF, and since the previous commit a tool reaching no verdict is an inviolable `CIT-LOC-002` - which would have hard-blocked every citing manuscript on such a machine. `verify_locators.py` now raises `GateUnavailable`, printing `GATE_UNAVAILABLE:` and exiting 3, and the auditor maps that to a `default` `CIT-LOC-001`. An author cannot fix a missing dependency by citing better. A crash with no marker at all is unchanged and still blocks.

Tests: `test_citations` 16/16, two added (bundled resolution order plus the PROVENANCE drift guard; GATE_UNAVAILABLE as advisory against a markerless crash as inviolable). The vendored `reconcile_citations_test.py` passes 5/5 in place. Honest limit: the bundled `verify_locators.py` needs `pymupdf`, so a bare install reports `CIT-LOC-001` rather than binding quotes until that is present.

**Citation evidence must reach a verdict, and must be about the manuscript being audited (20 September 2026).** An independent cross-family review (Codex, 2026-09-20) reproduced eight defects in the citation work; these are the two rated most consequential plus one bypass of the `1528a2b` symmetry fix. All three were reproduced here before being repaired.

*A crash in either gate tool now blocks.* `audit_source_locators` checked for a missing summary line only on `verify_locators.py`. `validate_sources.py`'s exit status was ignored unless its stdout happened to carry a recognised finding, so a crash in it returned **zero findings** while quote verification succeeded - silently clearing Rule 3 items 9-12 (identity, version, page convention, retraction, empty-reference, counter-evidence). Each tool now has a completion marker in `GATE_COMPLETION_RE` (`checks: N bound / M failed / T total`; `validation (item 9/10): N sources`) and a tool that never prints its marker raises an inviolable `CIT-LOC-002`. Exit status alone cannot decide this: a Python traceback also exits 1, which is a legitimate verdict code for both tools. `GATE_TOOLS` now runs validation first, as Rule 3 item 9 requires.

*Evidence is bound to the audited bytes.* The tools read only the checks file, so a checks set built for another manuscript cleared this one. Reproduced with real evidence declaring `citing_document.sha256 2ae9f90c...` beside a manuscript hashing `a807c8b9...` and asserting "All autonomous systems are always safe": zero inviolable findings. The auditor now passes `--citing-document <target>` to `verify_locators.py`, which fails when the document's sha256 differs from `citing_document.sha256`, fails any check whose `claim_text` is absent from that document, and reports every cited sentence no check covers as `UNCOVERED`. A checks set that binds no manuscript is `UNBOUND` unless it records `citing_document.not_a_manuscript` with a written reason, which is printed as a NOTE.

*Undated citations are citations.* `SOURCE_CITATION_RE` required a numeric year, so a manuscript whose citations were all `(Author, n.d.)` presented no citation at all and the inviolable `CIT-LOC-000` never fired - a one-keystroke bypass of the symmetry fix shipped hours earlier.

Tests: `test_citations` 14/14, four cases added. The stub helper was rewritten to control each tool separately; the previous one emitted identical output for both, which is precisely why the suite missed the asymmetric crash. `test_audit`, `draft_governance_smoketest` and `product_assurance_smoketest` unchanged; `contract-kernel-check` PASS at the unchanged `35d785ed5c45`. Not addressed here, and still open from the same review: PARAPHRASE ignores derived-term coverage; wiki `quote_bound` is page-level and survives replacement of the source PDF; `reconcile_citations.py` mishandles undated citations, author initials and duplicate numeric labels. Installed harnesses do not ship the gate tools, so they emit advisory `CIT-LOC-001` and no inviolable finding: this enforcement is not automatic on a bare install.

**Absence of citation evidence blocks exactly like failed citation evidence (19 September 2026).** `audit_source_locators` in `scripts/audit/audit_citations.py` treated the two states asymmetrically: a manuscript whose citations had all FAILed the quote-binding gate raised `CIT-LOC-010` at `inviolable`, while a manuscript that bound nothing at all raised `CIT-LOC-000` at `default` and passed. The cheapest route past the gate was therefore never to run it, which is the self-attestation loop Rule 4a exists to close, re-entered through the pre-flight's own severity table. `CIT-LOC-000` is now `inviolable`. Raising it alone would have shipped a blocker with a 30-byte bypass, so two further no-evidence states are now `inviolable` under the new `CIT-LOC-002`: a checks file the gate cannot read (both tools crash, exit non-zero, print nothing, and the auditor previously parsed no lines and emitted no finding) and a checks file the gate reads cleanly while binding zero checks (`checks: 0 bound / 0 failed / 0 total`, exit 0, previously indistinguishable from a clean pass). Detection is the `verify_locators.py` summary line (`GATE_SUMMARY_RE`): no summary at all, or a summary reporting zero checks, means no citation carries a bound quote. `CIT-LOC-002` is scoped to an *empty* file specifically — a file whose checks all failed is already reported once per failure as `CIT-LOC-010`, and adding "binds no checks at all" to it would be both duplicative and untrue. That was caught on a real 26-check file, and the case is now a test. Two findings stay `default` deliberately and the test asserts it: `CIT-LOC-001`, the gate tools are absent from `CITATION_GATE_TOOLS`, an environment fault an author cannot fix by working better and one that would otherwise block every installation without those private scripts; and `CIT-LOC-011`, an UNSUPPORTED element, which is a disclosed gap under Rule 3 item 8 (a closed-access source, say) rather than a skipped gate. Tests: `test_citations` 10/10, with `test_absent_evidence_blocks_exactly_like_failed_evidence` covering all five states through hermetic stub gate tools, so the suite no longer depends on the private tools being installed; `test_audit`, `draft_governance_smoketest` and `product_assurance_smoketest` unchanged. No hash-bound surface was touched: `contract-kernel-check` PASS at the unchanged `35d785ed5c45`, `manifest-coherence-check` 0 blockers. Consequence to expect: a citing manuscript with no harness-visible checks file now raises an inviolable finding, and `scripts/draft_governance.py` maps inviolable to MAJOR.

**Grounding Protocol Rule 4a: bind the quote before any citation verdict (19 September 2026).** `references/GROUNDING_PROTOCOL.md` gains Rule 4a (`gp-4a`). Before it, every citation verdict in the protocol (`CLEAN`, `PASS`, `Verified`, `supported`) rested on an agent having read the source; lesson L-P4 had recorded in April that such an audit passes a compound claim whose second conjunct the source does not support. Rule 4a forbids a citation verdict without a script-bound verbatim quote on the stated printed page for each claim element, separates validation (identity, version, page convention, dated retraction lookup) from verification (four support classes, inferences disclosed to the author), names wiki pages, graph nodes, abstracts, resolving DOIs, prior verdicts and the wiki's `eligibility: verified` as secondary, binds a verdict to the manuscript's sha256, states that a Rule 7a `MATCH` does not satisfy it, and blocks G.4 sign-off and a `CLEAN` grounding audit while the pre-flight carries `CIT-LOC-000`, `-001` or `-010`. The file is kernel-bound, so the kernel was refreshed and re-pinned in the shipment-v2 projection and profile. `grounding_anchors_check` reports 9 rules.

**Terminal check replays v2 draft-governance locators with their own validator (WO-05c, 19 September 2026).** `check_terminal` in `scripts/full_run_contract_check.py` sent every draft-policy locator to `validate_lifecycle_verifier_binding`. A locator with `binding_type: lifecycle_draft_governance` was therefore refused on schema grounds (`LIFECYCLE-EVIDENCE-INVALID`, additional properties) before any lifecycle fact was examined, and the receipt, dispatch-claim, dispatch-consumption and generation-result linkage was never checked at that gate. Such locators now go to `validate_lifecycle_draft_governance_binding` with the expected role, milestone, assignment receipt id, dispatch claim and consumption paths, and the generation result carried into the evaluation check; `DraftGovernanceLifecycleError` maps to `FRC-DRAFT-POLICY-INVALID`. This is the hunk WO-05b (`3c04c68`) held back from `wip/paper2-lifecycle-authoring` (`d5fe3c0`) for WO-05c, ported by hand because main's function had moved; two guards were added so a dispatch binding without a string `path` is reported as malformed instead of raising `KeyError` out of the check. `assignment_milestone_checkpoint_smoketest.py` gains two assertions: the terminal check reaches the draft-governance validator for a v2 project (fails against the unported file), and an absent dispatch binding is refused as malformed. Also refreshed: `references/semantics_manifest.v1.json` still carried the pre-`41b0025` hash of `scripts/bibliography_review.py`, so every verifier replay refused with "semantics member is stale" and `assignment_milestone_checkpoint_smoketest`, `production_evidence_publisher_smoketest` and `assignment_terminal_close_smoketest` could not pass between `41b0025` and this change.

**Source-citation evidence in the pre-flight, and passage-scoped quote matching (19 September 2026).** `scripts/audit/run_all.py` registers a new `source_locators` auditor (`audit_source_locators` in `scripts/audit/audit_citations.py`). Before this, the mandated pre-flight ran seven style/craft auditors and no check on a manuscript's citations to sources. The auditor issues no verdict itself: it relays the conclusions of the two external quote-binding gates run on `reviews/citation_checks.json` (`CIT-LOC-010` gate FAIL, inviolable; `-011` unsupported element; `-012` MAPPED/EXTENDED inference to disclose; `-013` validation REVIEW), emits `CIT-LOC-000` when a manuscript cites sources but has no checks file, and `CIT-LOC-001` when the gate tools are absent. It makes no network call. `scripts/bibliography_review.py` no longer matches an inspection or support quote against a whole bound document: when a material supplies no `passage_text(s)`, its file is accepted as the passage only if it is excerpt-sized (`EXCERPT_MAX_CHARS = 20000`); otherwise `BIBLIOGRAPHY-MATERIAL-UNSCOPED`. Tests: `test_citations` 9/9 (one added), `piw_research_support_smoketest` with a new refusal test, and the dependent smoketests unchanged. Not changed: `scripts/source_extract.py` still numbers pages by PDF index; its page map is byte-compared against committed fixture assets and hash-bound in project evidence, so adding printed-page labels needs a schema-versioned change. The "Coverage seam" paragraph of `skills/quick-deterministic/SKILL.md` now describes the `source_locators` auditor; because that file is hash-bound, `references/contract_kernel.v1.json` was refreshed and its hash re-pinned in `references/compatibility/shipment-v2/contract_kernel_projection.json` and `compatibility_profile.json` (kernel hash and kit aggregate), the same three-file re-pin earlier content commits made.

**Host-neutral native-child boundary (15 September 2026).** `scripts/piw_native_host.py` now dispatches through an adapter registry (`ADAPTERS`: `codex-jsonl`, `hermes-hooks-jsonl`, and the new `claude-code-jsonl` in `scripts/piw_claude_host.py`). The Claude adapter verifies Claude Code / Claude Desktop session JSONL: the dispatching `Agent` `tool_use` carrying `COAUTHOR_REQUEST_SHA256`, the `completed` `tool_result`, the child's `end_turn` final message under `<session>/subagents/agent-<id>.jsonl`, chronology, exact result JSON and pinned prefixes. Every coordinator request binds `role_prompt`, `skill_bodies` and `package_root` so children on any host read the same role file and skill bodies; agent file-resolution notes no longer assume `CLAUDE_PLUGIN_ROOT`. `piw_metrics.py` takes role start from the adapter. Fixtures: `scripts/claude_host_smoketest.py` (registered). Runbook: `references/CLAUDE_CODE_HOST.md`. Fixtures prove adapter mechanics only; live qualification needs a real session's own logs.
...
**C6 evaluates staged bytes (2026-08-21).** Dest-safe no longer occupies evaluate. C6 itself was still exiting on `SET-BINDING-MISSING` when a Generator envelope or `assignment_dispatch` tree was absent. That binding stays correct when a generator dispatch exists: a present stale or wrong envelope still refuses `SET-DISPATCH-SEPARATION` / `SET-ARTIFACT-STALE`. It is not a bar on claim, derivation, warrant, and citation of bytes Joseph already staged without a generator run. `scripts/scholarly_evaluation.py` now reads those named-milestone bytes and emits honest findings, including MAJOR. It does not invent a Generator envelope, assignment_dispatch tree, F9, or fake receipts, and it still cannot mint scholarly CLEAN without Evaluator fire and unresolved-finding close. File presence is never acceptance. FINAL still requires accepted M1-M4. No paper-specific bars.

**Lab-audit instrument repair (2026-08-21).** The reader-policy CLI and `audit/run_all.py` no longer report `GRAPH-SEMANTIC-INELIGIBLE` as `RA-POLICY` / `MISCONFIGURED`. That code is fail-closed eligibility; `--out` now retains the diagnostic. A reader-profile v2 project with `semantic_usage: not_invoked` resolves through `resolve_reader_profile` and does not invoke the semantic graph. Candidate generation now emits `reader_accessibility_candidates.v2` without semantic-view pins for that binding. Non-mapping `phase_state.json` containers return the controlled not-dormant result instead of raising. The shipment-v2 compatibility projection and kit tuple are rebound to the repaired kernel digest. Phase 0.6 authors F6 only under `full_lifecycle`; `lab_iteration` uses a distinct shipment-lane `GENERATION_PLAN.md` that is not F6. `R-Refl-DP-2` is `full_lifecycle`-only; an F6 path in `lab_iteration` is `R-Refl-DP-LAB-FORBIDDEN`. DEST-PROTECTED, SK-32 CLOSED, no scholarly CLEAN, and no Master Governance edit.

**Chung academic-voice pass (2026-08-21).** New optional skill **SK-49 `chung-academic-voice-pass`** plus `references/chung_academic_voice_guidelines.md`. On-demand audit or explicit draft overlay for the registered authorial register (technical representation joined to institutional consequences). Not a STYLE_COMMITMENTS C-n. Catalog, command-surface, and capability rows remain `degraded` / prompt-mediated. DEST-PROTECTED, SK-32 CLOSED, no scholarly CLEAN.

**M4 v2 evaluate-gap repair (2026-08-21).** Lab-iteration all-pass on named M4 v2 bytes filed honest MAJORs/MINORs but C6 then refused `SET-BINDING-MISSING` because no plain scholarly-profile file existed, and `draft_governance.py verify` mapped that to `DRAFT-POLICY-SCHOLARLY-EVALUATION-STALE`. Evaluation-lane now writes a dest-safe `c6/scholarly-profile.json` (plus a copied obligation registry) into the shipment lane. Evaluator must bind that exact file as `scholarly_profile`. The emit is not scholarly fire and does not mint CLEAN. Mechanical D-STYLE flags a multi-section opening with no §1 roadmap (MINOR). `check-abstract-body` Part C requires a roadmap. Evaluator fire tables add `check-abstract-body` and `chung-academic-voice-pass`, including the prose-requirement overlay. `question_type` / `citation_style` TBD stay MINOR; the harness still does not invent those values. DEST-PROTECTED, SK-32 CLOSED, no F9, no acceptance, no Writer apply.
