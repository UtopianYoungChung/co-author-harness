...
*Known open defects surfaced by the first CI registry runs.* The Windows job (`fb09fab`, before the fixes above) passed 78 of 114 cases. About 20 failures share one cause. The hosted runner's `TEMP` is an 8.3 alias (`C:\Users\RUNNER~1\…`), and resolvers such as `reader_accessibility_policy.resolve_domain_native_register` (`relative_to(wiki_root)`) compare resolved long paths with unresolved short ones and raise `ValueError`; the same mix is the likely cause of the `APG-RECEIPT-INVALID` receipt-path and `admitted root handle identity mismatch` failures. Any Windows user whose profile path is 8.3-shortened is exposed. CI now points `TEMP`/`TMP` at the runner's long-path temp directory, as on the author's host, but the resolvers still need to canonicalise both sides. `release_qualification_controller_smoketest` needs a detached worker to break away from its job object, which the hosted Windows runner's job object forbids ("detached Windows worker cannot escape the enclosing Job"). Suites that commit in scratch repositories needed a git identity, which CI now configures. With these changes the Windows job at `6a5ea15` passed 110 of 114 cases. `semantic_qualification_consumer_smoketest` is UNAVAILABLE (sibling wiki absent). `migrate_lab_iteration_derived_handoff_smoketest` passes 8 of 23 checks; the rest re-derive policy from the author's `knowledge/LLM wiki`. The release controller still meets the Job restriction. The fourth failure is fixed below.
