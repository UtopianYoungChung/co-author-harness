...
*Known open defect: Linux suite timeouts lose their diagnostic and output.* With transcript capture no longer refused, CI's Ubuntu `fixture_infrastructure_check` reaches the infrastructure-error case and fails 7 checks: a timed-out suite yields `FIXTURE-PROCESS-LIVE` ("systemd anchor result is absent") with empty stdout and stderr, where Windows yields `FIXTURE-PROCESS-TIMEOUT` with the captured bytes. In `fixture_process_supervisor.py` the parent's wait loop and the in-unit anchor share one `execution_deadline` (the deadline minus the cleanup reserve), and the unit's `RuntimeMaxSec` expires a moment later. The parent therefore declares the unit over budget and kills it at the instant the anchor begins its timeout teardown, and the anchor never gets the reserve it needs to report. A 3-second probe deadline did not change the result and was reverted. The fix belongs in the supervisor: give the anchor an execution deadline that leaves its reporting margin inside `RuntimeMaxSec`, or let the parent wait through the cleanup reserve for the anchor's result. It needs a host with a systemd user manager and cgroup v2 to verify.
